...
class TestInstallNBExtension(TestCase):
    ...
    def assert_installed(self, relative_path, user=False):
        ...
        self.assert_dir_exists(pjoin(nbext, relative_path))
    ...
    def assert_not_installed(self, relative_path, user=False):
        if user:
            ...
            nbext = self.system_nbext
        self.assert_not_dir_exists(pjoin(nbext, relative_path))
    ...
    def test_create_nbextensions_user(self):
        with TemporaryDirectory() as td:
            install_nbextension(self.src, user=True)
            self.assert_installed(pjoin(basename(self.src), u"ƒile"), user=True)
    ...
    def test_create_nbextensions_system(self):
        with TemporaryDirectory() as td:
            ...
            with patch.object(nbextensions, "SYSTEM_JUPYTER_PATH", [td]):
                ...
                self.assert_installed(pjoin(basename(self.src), u"ƒile"), user=False)
    ...
    def test_install_url(self):
        ...
        try:
            ...
            install_nbextension(
                "https://example.com/path/to/another/bar.js", destination="foobar.js"
            )
    ...
    def test_nbextensionpy_enable(self):
        ...
        enabled = (
            cm.get("notebook")
            .get("load_extensions", {})
            .get("_mockdestination/index", False)
        )
        assert enabled

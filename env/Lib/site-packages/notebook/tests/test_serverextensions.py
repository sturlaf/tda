...
class MockEnvTestCase(TestCase):
    ...
    def setUp(self):
        ...
        for mod in (paths, extensions):
            p = patch.object(mod, "SYSTEM_CONFIG_PATH", self.system_config_path)
            self.patches.append(p)
        ...
        # verify our patches
        self.assertEqual(
            paths.jupyter_config_path(), [self.config_dir] + self.system_config_path
        )
...
class TestOrderedServerExtension(MockEnvTestCase):
    ...
    def test_load_ordered(self):
        app = NotebookApp()
        app.nbserver_extensions = OrderedDict(
            [("mockextension2", True), ("mockextension1", True)]
        )

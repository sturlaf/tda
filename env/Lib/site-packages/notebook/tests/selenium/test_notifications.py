...
def test_notification(notebook):
    ...
    assert get_widget(notebook, "kernel") and widget(
        notebook, "kernel"
    ), "The kernel notification widget exists"
    assert get_widget(notebook, "notebook") and widget(
        notebook, "notebook"
    ...
    for level in ("info", "warning", "danger"):
        ...
        assert (
            widget_message(notebook, "test") == f"test {level}"
        ), f"{level}: message is correct"
    ...
    assert (
        widget_message(notebook, "test") == "test timeout"
    ...
    assert (
        widget_message(notebook, "test") == "test click"
    ), "callback: message is correct"

    notebook.browser.find_element_by_id("notification_test").click()
    wait_for_script_to_return_true(
        notebook.browser, 'return IPython.notification_area.widget("test")._clicked;'
    )
    wait_for_selector(notebook.browser, "#notification_test", obscures=True)

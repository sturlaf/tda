...
@pytest.mark.skipif(
    not bool(os.environ.get("RUN_NB_INTEGRATION_TESTS", False)),
...
def test_stop_multi_integration():
    ...
    # Default port.
    p1 = subprocess.Popen(["jupyter-notebook", "--no-browser"])

    # Unix socket.
    sock = UNIXSocketNotebookTestBase.sock
    p2 = subprocess.Popen(["jupyter-notebook", "--sock=%s" % sock])

    # Specified port
    p3 = subprocess.Popen(["jupyter-notebook", "--no-browser", "--port=%s" % TEST_PORT])

    time.sleep(3)

    assert (
        MSG_TMPL.format(DEFAULT_NOTEBOOK_PORT)
        in subprocess.check_output(["jupyter-notebook", "stop"]).decode()
    )

    _ensure_stopped("There is currently no server running on 8888")

    assert (
        MSG_TMPL.format(sock)
        in subprocess.check_output(["jupyter-notebook", "stop", sock]).decode()
    )

    assert (
        MSG_TMPL.format(TEST_PORT)
        in subprocess.check_output(["jupyter-notebook", "stop", TEST_PORT]).decode()
    )

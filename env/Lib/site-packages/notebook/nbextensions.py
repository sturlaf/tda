...
def check_nbextension(
    files, user=False, prefix=None, nbextensions_dir=None, sys_prefix=False
):
...
def uninstall_nbextension_python(
    module,
    user=False,
    sys_prefix=False,
    prefix=None,
...
def _set_nbextension_state(
    section, require, state, user=True, sys_prefix=False, logger=None
):
...
def _set_nbextension_state_python(state, module, user, sys_prefix, logger=None):
...
def enable_nbextension(section, require, user=True, sys_prefix=False, logger=None):
...
def disable_nbextension(section, require, user=True, sys_prefix=False, logger=None):
...
def enable_nbextension_python(module, user=True, sys_prefix=False, logger=None):
    ...
    logger : Jupyter logger [optional]
        Logger instance to use
    """
    return _set_nbextension_state_python(True, module, user, sys_prefix, logger=logger)


def disable_nbextension_python(module, user=True, sys_prefix=False, logger=None):
    ...
    logger : Jupyter logger [optional]
        Logger instance to use
    """
    return _set_nbextension_state_python(False, module, user, sys_prefix, logger=logger)
...
def validate_nbextension_python(spec, full_dest, logger=None):
    ...
    if require is not None:
        require_path = os.path.join(
            full_dest[0 : -len(spec["dest"])], u"{}.js".format(require)
        )
...
class UninstallNBExtensionApp(BaseExtensionApp):
    ...
    flags["system"] = (
        {"UninstallNBExtensionApp": {"system": True}},
    ...
    def find_uninstall_extension(self):
        ...
        if self.python:
            ...
            for nbext in nbexts:
                ...
                for section in NBCONFIG_SECTIONS:
                    _find_disable_nbextension(
                        section, nbext["require"], logger=self.log
                    )
...
class ToggleNBExtensionApp(BaseExtensionApp):
    ...
    def toggle_nbextension_python(self, module):
        ...
        return toggle(
            module, user=self.user, sys_prefix=self.sys_prefix, logger=self.log
        )
    ...
    def toggle_nbextension(self, require):
        ...
        """
        toggle = enable_nbextension if self._toggle_value else disable_nbextension
        return toggle(
            self.section,
            require,
            user=self.user,
            ...
            for section in NBCONFIG_SECTIONS:
                ...
                if "load_extensions" in data:
                    ...
                    for require, enabled in data["load_extensions"].items():
                        print(
                            u"      {} {}".format(
                                require, GREEN_ENABLED if enabled else RED_DISABLED
                            )
                        )
                        if enabled:

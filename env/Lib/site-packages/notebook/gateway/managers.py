...
async def gateway_request(endpoint, **kwargs):
    ...
    # of the tree view.
        ...
        raise web.HTTPError(
            ...
            "url is valid and the Gateway instance is running.".format(
                GatewayClient.instance().url
            ),
        ) from e
    ...
    async def get_kernel(self, kernel_id=None, **kwargs):
        ...
        try:
            ...
            if kernel_id in self._kernels:
                ...
                self.log.warning(
                    "Kernel '%s' is not managed by this instance.", kernel_id
                )
                kernel = None
        return kernel
    ...
    def shutdown_all(self, now=False):
        ...
        for kernel_id in self._kernels:
            ...
            try:
                ...
                self.log.debug(
                    "Delete kernel response: %d %s", response.code, response.reason
                )
            shutdown_kernels.append(
                kernel_id
...
class GatewayKernelSpecManager(KernelSpecManager):
    ...
    async def get_all_specs(self):
        ...
        if remote_default_kernel_name != km.default_kernel_name:
            self.log.info(
                "Default kernel name on Gateway server ({gateway_default}) differs from "
                "Notebook server ({notebook_default}).  Updating to Gateway server's value.".format(
                    gateway_default=remote_default_kernel_name,
    ...
    async def get_kernel_spec_resource(self, kernel_name, path):
        ...
        self.log.debug(
            "Request kernel spec resource '{}' at: {}".format(

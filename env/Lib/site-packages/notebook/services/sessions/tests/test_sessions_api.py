...
class SessionAPI(object):
    ...
    def _req(self, verb, path, body=None):
        response = self.request(verb, url_path_join("api/sessions", path), data=body)
    ...
    def create_deprecated(self, path):
        body = json.dumps(
            {"notebook": {"path": path}, "kernel": {"name": "python", "id": "foo"}}
        )
        return self._req("POST", "", body)
...
class SessionAPITest(NotebookTestBase):
    ...
    def test_create(self):
        ...
        self.assertEqual(
            ...
            self.url_prefix + "api/sessions/{0}".format(newsession["id"]),
        )
...
class AsyncSessionAPITest(SessionAPITest):
    ...
    def setup_class(cls):
        if (
            ...
            raise SkipTest(
                "AsyncSessionAPITest tests skipped due to down-level jupyter_client!"
            )

...
class TestSessionManager(TestCase):
    ...
    def test_get_session(self):
        sm = self.sm
        session_id = self.create_session(path="/path/to/test.ipynb", kernel_name="bar")[
            "id"
        ]
    ...
    def test_bad_get_session(self):
        ...
        sm = self.sm
        session_id = self.create_session(path="/path/to/test.ipynb", kernel_name="foo")[
            "id"
        ]
    ...
    def test_update_session(self):
        ...
        session_id = self.create_session(
            path="/path/to/test.ipynb", kernel_name="julia"
        )["id"]
        self.loop.run_sync(
            lambda: sm.update_session(session_id, path="/path/to/new_name.ipynb")
        )
        model = self.loop.run_sync(lambda: sm.get_session(session_id=session_id))
        expected = {
            "id": session_id,
    ...
    def test_bad_update_session(self):
        ...
        sm = self.sm
        session_id = self.create_session(path="/path/to/test.ipynb", kernel_name="ir")[
            "id"
        ]
        with self.assertRaises(TypeError):
            self.loop.run_sync(
                lambda: sm.update_session(session_id=session_id, bad_kw="test.ipynb")
            )  # Bad keyword
    ...
    def test_bad_delete_session(self):
        ...
        with self.assertRaises(TypeError):
            self.loop.run_sync(
                lambda: sm.delete_session(bad_kwarg="23424")
            )  # Bad keyword
        with self.assertRaises(web.HTTPError):
            self.loop.run_sync(
                lambda: sm.delete_session(session_id="23424")
            )  # nonexistent

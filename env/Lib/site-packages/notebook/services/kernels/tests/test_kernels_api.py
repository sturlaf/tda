...
class KernelAPI(object):
    ...
    def _req(self, verb, path, body=None):
        response = self.request(verb, url_path_join("api/kernels", path), data=body)
...
class KernelAPITest(NotebookTestBase):
    ...
    def test_default_kernel(self):
        ...
        self.assertEqual(
            r.headers["location"],
            url_path_join(self.url_prefix, "api/kernels", kern1["id"]),
        )
...
class AsyncKernelAPITest(KernelAPITest):
    ...
    def setup_class(cls):
        if (
            not async_testing_enabled
        ):  # Can be removed once jupyter_client >= 6.1 is required.
            raise SkipTest(
                "AsyncKernelAPITest tests skipped due to down-level jupyter_client!"
            )
...
class KernelFilterTest(NotebookTestBase):
    ...
    def test_config(self):
        self.assertEqual(
            self.notebook.kernel_manager.allowed_message_types, ["kernel_info_request"]
        )
...
class KernelCullingTest(NotebookTestBase):
    ...
    def get_argv(cls):
        ...
        argv.extend(
            [
                "--MappingKernelManager.cull_idle_timeout={}".format(CULL_TIMEOUT),

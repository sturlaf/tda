...
try:
    ...
    raise ImportError(
        _("The Jupyter Notebook requires tornado >= 5.0, but you have < 1.1.0")
    ) from e
if version_info < (5, 0):
    raise ImportError(
        _("The Jupyter Notebook requires tornado >= 5.0, but you have %s")
        % tornado.version
    )
...
class NotebookWebApplication(web.Application):
    def __init__(
        ...
        if settings["autoreload"]:
            log.info(
                "Autoreload enabled: the webapp will restart when any Python src file changes."
            )
    ...
    def init_settings(
        ...
        nbui = gettext.translation(
            "nbui", localedir=os.path.join(base_dir, "notebook/i18n"), fallback=True
        )
    ...
    watch and build the notebook's JavaScript for you, as you make changes."""
        ...
        home = py3compat.str_to_unicode(
            os.path.expanduser("~"), encoding=sys.getfilesystemencoding()
        )
    ...
    if url.startswith("http+unix://"):
        ...
        class UnixSocketResolver(Resolver):
            ...
            def resolve(self, host, port, *args, **kwargs):
                raise gen.Return([(socket.AF_UNIX, urldecode_unix_socket_path(host))])
...
class NbserverStopApp(JupyterApp):
    ...
    def start(self):
        ...
        for server in servers:
            ...
            for server in servers:
                print(
                    "  - {}".format(server.get("sock") or server["port"]),
                    file=sys.stderr,
                )
            self.exit(1)
...
class NbserverListApp(JupyterApp):
    ...
    flags = dict(
        jsonlist=(
            {"NbserverListApp": {"jsonlist": True}},
            _("Produce machine-readable JSON list output."),
        ),
        json=(
            {"NbserverListApp": {"json": True}},
...
class NotebookApp(JupyterApp):
    ...
    log_json = Bool(
        ...
        config=True,
        help=_(
            "Set to True to enable JSON formatted logs. "
    ...
    default_url = Unicode(
        "/tree", config=True, help=_("The default URL to redirect to from `/`")
    )
    ...
    sock = Unicode(
        u"", config=True, help=_("The UNIX socket the notebook server will listen on.")
    )
    ...
    webapp_settings = Dict(config=True, help=_("DEPRECATED, use tornado_settings"))
    ...
    def init_configurables(self):
        ...
        if self.gateway_config.gateway_enabled:
            ...
            self.session_manager_class = (
                "notebook.gateway.managers.GatewaySessionManager"
            )
            self.kernel_spec_manager_class = (
                "notebook.gateway.managers.GatewayKernelSpecManager"
            )
        ...
        if isinstance(self.kernel_manager, AsyncMappingKernelManager):
            if (
                not async_kernel_mgmt_available
            ):  # Can be removed once jupyter_client >= 6.1 is required.
                raise ValueError(
                    "You are using `AsyncMappingKernelManager` without an appropriate "
                    "jupyter_client installed!  Please upgrade jupyter_client or change kernel managers."
                )
            self.log.info(
                "Asynchronous kernel management has been configured to use '{}'.".format(
                    self.kernel_manager.__class__.__name__
                )
            )
    ...
    def init_webapp(self):
        ...
        if self.password_required and (not self.password):
            self.log.critical(
                _("Notebook servers are configured to only be run with a password.")
            )
        ...
        if self.sock:
            ...
            if self.file_to_run:
                self.log.critical(
                    _(
                        "Options --NotebookApp.file_to_run and --sock are mutually exclusive."
                    ),
                )
                sys.exit(1)
    ...
    def _bind_http_server_unix(self):
        ...
        try:
            ...
            if e.errno == errno.EADDRINUSE:
                ...
                self.log.warning(
                    _("Permission to listen on sock %s denied") % self.sock
                )
                return False
            else:
                raise
            ...
            try:
                ...
                if e.errno == errno.EADDRINUSE:
                    if self.port_retries:
                        self.log.info(
                            _("The port %i is already in use, trying another port.")
                            % port
                        )
                    else:
                    ...
                    self.log.warning(
                        _("Permission to listen on port %i denied.") % port
                    )
                    continue
                else:
                    raise
    ...
    def init_server_extension_config(self):
        ...
        for modulename, enabled in sorted(extensions.items()):
            if modulename not in self.nbserver_extensions:
                self.config.NotebookApp.nbserver_extensions.update(
                    {modulename: enabled}
                )
    ...
    def notebook_info(self, kernel_count=True):
        ...
        info += _(
            "Jupyter Notebook {version} is running at:\n{url}".format(
                version=NotebookApp.version, url=self.display_url
            )
        )
        if self.gateway_config.gateway_enabled:
            info += (
                ...
                self.log.critical(
                    "\n".join(
                        [
                            ...
                            % (
                                self.sock,
                                self._concat_token(self._tcp_url("localhost", 8888)),
                            ),
                        ]
                    )
                )
                    ...
                    self.log.critical(
                        "\n".join(
                            [
                                ...
                                "    %s"
                                % urljoin(
                                    "file:", pathname2url(self.browser_open_file)
                                ),

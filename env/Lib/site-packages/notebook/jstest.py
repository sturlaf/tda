...
class JSController(TestController):
    ...
    def setup(self):
        ...
        if self.url:
            ...
            if self.server_port:
                self.cmd.append(
                    "--url=http://localhost:%i%s" % (self.server_port, self.base_url)
                )
            else:
    ...
    def _wait_for_server(self):
        ...
        for i in range(300):
            ...
            time.sleep(0.1)
        print(
            "Notebook server-info file never arrived: %s" % self.server_info_file,
            file=sys.stderr,
        )
    ...
    def cleanup(self):
        if hasattr(self, "server"):
            ...
            try:
                ...
                try:
                    print(
                        "Failed to terminate notebook server, killing it.",
                        file=sys.stderr,
                    )
                    self.server.kill()
                except OSError:
            ...
            try:
                ...
                print(
                    "Notebook server still running (%s)" % self.server_info_file,
                    file=sys.stderr,
                )

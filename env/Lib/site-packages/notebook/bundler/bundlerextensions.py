...
def _get_bundler_metadata(module):
    ...
    if not hasattr(m, "_jupyter_bundlerextension_paths"):
        raise KeyError(
            "The Python module {} does not contain a valid bundlerextension".format(
                module
            )
        )
...
def _set_bundler_state(
    name, label, module_name, group, state, user=True, sys_prefix=False, logger=None
):
    ...
    if state:
        cm.update(
            ...
            {
                BUNDLER_SUBSECTION: {
                    name: {"label": label, "module_name": module_name, "group": group}
                }
            },
        )
    else:
    ...
    return (
        cm.get(BUNDLER_SECTION).get(BUNDLER_SUBSECTION, {}).get(name) is not None
    ) == state
...
def enable_bundler_python(module, user=True, sys_prefix=False, logger=None):
    ...
    logger : Jupyter logger [optional]
        Logger instance to use
    """
    return _set_bundler_state_python(True, module, user, sys_prefix, logger=logger)
...
def disable_bundler_python(module, user=True, sys_prefix=False, logger=None):
    ...
    logger : Jupyter logger [optional]
        Logger instance to use
    """
    return _set_bundler_state_python(False, module, user, sys_prefix, logger=logger)
...
class ToggleBundlerExtensionApp(BaseExtensionApp):
    ...
    def toggle_bundler_python(self, module):
        ...
        module : str
            ...
            magic-named `_jupyter_bundlerextension_paths` function
        """
        toggle = enable_bundler_python if self._toggle_value else disable_bundler_python
        return toggle(
    ...
    def start(self):
        ...
        if self.python:
            ...
            raise NotImplementedError(
                "Cannot install bundlers from non-Python packages"
            )

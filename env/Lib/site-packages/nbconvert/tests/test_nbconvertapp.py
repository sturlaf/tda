...
class TestNbConvertApp(TestsBase):
    ...
    def test_glob_subdir(self):
        ...
        with self.create_temp_cwd():
            ...
            self.nbconvert(
                "--to python --log-level 0 " + os.path.join("subdir", "*.ipynb")
            )
    ...
    def test_build_dir(self):
        ...
        with self.create_temp_cwd():
            self.copy_files_to(["notebook*.ipynb"], "subdir/")
            self.nbconvert(
                "--to python --log-level 0 --output-dir . "
    ...
    def test_absolute_template_file(self):
        ...
        with self.create_temp_cwd(
            ["notebook*.ipynb"]
        ), tempdir.TemporaryDirectory() as td:
            template = os.path.join(td, "mytemplate.tpl")
            test_output = "success!"
            with open(template, "w") as f:
                f.write(test_output)
            self.nbconvert(
                "--log-level 0 notebook2 --to html --template-file %s" % template
            )
    ...
    def test_output_ext(self):
        ...
        with self.create_temp_cwd(["notebook1.ipynb"]):
            self.nbconvert(
                "--log-level 0 --to python " "notebook1.ipynb --output nb.py"
            )
    ...
    def test_glob_explicit(self):
        ...
        with self.create_temp_cwd(["notebook*.ipynb"]):
            self.nbconvert(
                "--log-level 0 --to python " "*.ipynb notebook1.ipynb notebook2.ipynb"
            )
            assert os.path.isfile("notebook1.py")
            assert os.path.isfile("notebook2.py")
    ...
    def test_explicit_glob(self):
        ...
        with self.create_temp_cwd(["notebook*.ipynb"]):
            self.nbconvert(
                "--log-level 0 --to=python " "notebook1.ipynb notebook2.ipynb *.ipynb"
            )
            assert os.path.isfile("notebook1.py")
            assert os.path.isfile("notebook2.py")
    ...
    def test_cwd_plugin(self):
        ...
        with self.create_temp_cwd(["hello.py"]):
            ...
            self.nbconvert(
                "empty --to html --NbConvertApp.writer_class='hello.HelloWriter'"
            )
    ...
    def test_different_build_dir(self):
        ...
        with self.create_temp_cwd():
            ...
            os.mkdir("output")
            self.nbconvert(
                "empty.ipynb --to notebook " "--FilesWriter.build_directory=output"
            )
    ...
    def test_errors_print_traceback(self):
        ...
        with self.create_temp_cwd(["notebook3_with_errors.ipynb"]):
            ...
            assert (
                'raise RuntimeError("This is a deliberate exception")' in error_output
            )
    ...
    def test_convert_from_stdin_to_stdout(self):
        ...
        with self.create_temp_cwd(["notebook1.ipynb"]):
            with io.open("notebook1.ipynb") as f:
                ...
                output1, _ = self.nbconvert(
                    "--to markdown --stdin --stdout", stdin=notebook
                )
            assert "```python" not in output1  # shouldn't have language
            assert "```" in output1  # but should have fenced blocks
    ...
    def test_markdown_display_priority(self):
        ...
        with self.create_temp_cwd(["markdown_display_priority.ipynb"]):
            self.nbconvert(
                "--log-level 0 --to markdown " '"markdown_display_priority.ipynb"'
            )
            ...
            with open("markdown_display_priority.md", "r", encoding="utf8") as f:
                ...
                assert (
                    "markdown_display_priority_files/"
                    "markdown_display_priority_0_1.png"
                ) in markdown_output

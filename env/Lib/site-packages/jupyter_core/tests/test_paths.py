...
def test_envset():
    ...
    with patch.dict(
        "os.environ", ((f"FOO_{v}", v) for v in true_values + false_values)
    ):
...
def test_runtime_dir_windows():
    runtime = jupyter_runtime_dir()
    assert runtime == realpath(
        pjoin(os.environ.get("APPDATA", None), "jupyter", "runtime")
    )
...
@pytest.mark.skipif(
    not (
        sys.platform == "win32"
        and (
            ("__pypy__" not in sys.modules) or (sys.implementation.version >= (7, 3, 6))
        )
    ),
...
def test_is_hidden_win32_pypy():
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert "hidden files are not detectable on this system" in str(
                w[-1].message
            )

...
def get_active_backend(prefer=None, require=None, verbose=0):
    ...
    if prefer not in VALID_BACKEND_HINTS:
        raise ValueError(
            "prefer=%r is not a valid backend hint, "
            "expected one of %r" % (prefer, VALID_BACKEND_HINTS)
        )
    if require not in VALID_BACKEND_CONSTRAINTS:
        raise ValueError(
            "require=%r is not a valid backend constraint, "
            "expected one of %r" % (require, VALID_BACKEND_CONSTRAINTS)
        )
        ...
        if require == "sharedmem" and not supports_sharedmem:
            ...
            if verbose >= 10:
                print(
                    ...
                    % (sharedmem_backend.__class__.__name__, backend.__class__.__name__)
                )
...
class parallel_backend(object):
    ...
    def __init__(
        ...
        if inner_max_num_threads is not None:
            msg = (
                "{} does not accept setting the inner_max_num_threads "
...
class Parallel(Logger):
    ...
    * Ability to use shared memory efficiently with worker
    ...
    def __call__(self, iterable):
        ...
        self._print(
            "Using backend %s with %d concurrent workers.", (backend_name, n_jobs)
        )

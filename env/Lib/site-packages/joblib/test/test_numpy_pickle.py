...
def test_numpy_persistence(tmpdir, compress):
    ...
    for obj in (
        ...
        np.memmap(filename + "mmap", mode="w+", shape=4, dtype=np.float),
    ):
        filenames = numpy_pickle.dump(obj, filename, compress=compress)
        # All is cached in one file
        assert len(filenames) == 1
...
def test_compress_mmap_mode_warning(tmpdir):
    ...
    assert str(
        warninfo[0].message
    ) == 'mmap_mode "%(mmap_mode)s" is not compatible with compressed ' 'file %(filename)s. "%(mmap_mode)s" flag will be ignored.' % {
...
def test_cache_size_warning(tmpdir, cache_size):
    ...
    for w in warninfo:
        ...
        assert (
            str(w.message) == "Please do not set 'cache_size' in joblib.dump, this "

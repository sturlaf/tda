...
base = (
    ...
    .encode(
        x=alt.X("scaled_date:Q", axis=alt.Axis(title="Year into Decade", tickCount=11)),
...
start_year = text.transform_filter(alt.datum.end == "first").mark_text(baseline="top")

end_year = text.transform_filter(alt.datum.end == "last").mark_text(baseline="bottom")

(line + start_year + end_year).configure_text(align="left", dx=1, dy=3).properties(

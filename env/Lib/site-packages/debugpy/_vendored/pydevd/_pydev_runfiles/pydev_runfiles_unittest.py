...
class PydevTestResult(_PythonTextTestResult):
    def addSubTest(self, test, subtest, err):
        ...
        if err is not None:
            ...
            self._reportErrors(
                [error], [], "", "%s %s" % (self.get_test_name(test), subdesc)
            )
    ...
    def get_test_name(self, test):
        try:
            try:
                ...
                try:
                    test_name = (
                        test.__class__.__name__ + "." + test._TestCase__testMethodName
                    )
                # Support for class/module exceptions (test is instance of _ErrorHolder)
                except:
                    test_name = (
                        test.description.split()[1][1:-1]
                        + " <"
    ...
    def addError(self, test, err):
        ...
        # Support for class/module exceptions (test is instance of _ErrorHolder)
        if (
            not hasattr(self, "_current_errors_stack")
            or test.__class__.__name__ == "_ErrorHolder"
        ):

...
def execute_tests_in_parallel(
    tests, jobs, split, verbosity, coverage_files, coverage_include
):
    """
    ...
    for test_cases in queue_elements:
        ...
        for test_case in test_cases:
            try:
                test_name = (
                    test_case.__class__.__name__ + "." + test_case._testMethodName
                )
                ...
                test_name = (
                    ...
                    + "."
                    + test_case._TestCase__testMethodName
                )
    ...
    for i in range(jobs):
        ...
        if coverage_files:
            clients.append(
                ClientThread(
                    i, port, verbosity, coverage_files.pop(0), coverage_include
                )
            )
        else:
...
class CommunicationThread(threading.Thread):
    def __init__(self, tests_queue):
        ...
        server = SimpleXMLRPCServer(
            (pydev_localhost.get_localhost(), 0), logRequests=False
        )
...
class ClientThread(threading.Thread):
    def __init__(
        self, job_id, port, verbosity, coverage_output_file=None, coverage_include=None
    ):
        threading.Thread.__init__(self)
        self.daemon = True
        self.port = port
        ...
        try:
            ...
            if False:
                ...
                thread.start_new_thread(
                    target=self._reader_thread, args=(proc.stderr, sys.stderr)
                )
            else:

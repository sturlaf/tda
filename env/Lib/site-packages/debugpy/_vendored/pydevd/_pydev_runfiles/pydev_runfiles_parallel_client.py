...
class ServerComm(threading.Thread):
    def __init__(self, job_id, server):
        ...
        self.setDaemon(
            False
        )  # Wait for all the notifications to be passed before exiting!
...
class ServerFacade(object):
    ...
    def notifyStartTest(self, *args, **kwargs):
        self.notifications_queue.put_nowait(
            ParallelNotification("notifyStartTest", args, kwargs)
        )

    def notifyTest(self, *args, **kwargs):
        self.notifications_queue.put_nowait(
            ParallelNotification("notifyTest", args, kwargs)
        )
            ...
            while tests_to_run:
                ...
                for test in tests_to_run:
                    ...
                    if len(filename_and_test) == 2:
                        files_to_tests.setdefault(filename_and_test[0], []).append(
                            filename_and_test[1]
                        )
...
if __name__ == "__main__":
    if len(sys.argv) - 1 == 3:
        ...
        raise AssertionError(
            "Could not find out how to handle the parameters: " + sys.argv[1:]
        )

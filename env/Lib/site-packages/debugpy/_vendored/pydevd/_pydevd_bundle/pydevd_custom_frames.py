...
def add_custom_frame(frame, name, thread_id):
    ...
    with CustomFramesContainer.custom_frames_lock:
        ...
        next_id = CustomFramesContainer._next_frame_id = (
            CustomFramesContainer._next_frame_id + 1
        )
        ...
        CustomFramesContainer.custom_frames[frame_custom_thread_id] = CustomFrame(
...
def update_custom_frame(frame_custom_thread_id, frame, thread_id, name=None):
    with CustomFramesContainer.custom_frames_lock:
        ...
        try:
            ...
            sys.stderr.write(
                "Unable to get frame to replace: %s\n" % (frame_custom_thread_id,)
            )
            pydev_log.exception()

    ...
    if exceptions is not None:
        try:
            ...
            for exception_breakpoint in dict_iter_values(exceptions):
                if exception_breakpoint.type is not None and issubclass(
                    exctype, exception_breakpoint.type
                ):
...
def stop_on_unhandled_exception(py_db, thread, additional_info, arg):
    ...
    if exception_breakpoint.condition is not None:
        eval_result = py_db.handle_breakpoint_condition(
            additional_info, exception_breakpoint, user_frame
        )
        if not eval_result:
            return
    ...
    if exception_breakpoint.expression is not None:
        py_db.handle_breakpoint_expression(
            exception_breakpoint, additional_info, user_frame
        )
    ...
    pydev_log.debug(
        "Handling post-mortem stop on exception breakpoint %s"

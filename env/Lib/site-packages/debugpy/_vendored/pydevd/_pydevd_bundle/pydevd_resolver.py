...
TOO_LARGE_MSG = "Too large to show contents. Max items to show: " + str(
    MAX_ITEMS_TO_HANDLE
)
...
class DefaultResolver:
    ...
    def get_contents_debug_adapter_protocol(self, obj, fmt=None):
        ...
        lst = sorted(
            dict_iter_items(dct), key=lambda tup: sorted_attributes_key(tup[0])
        )
        if used___dict__:
...
class DictResolver:
    ...
    def get_contents_debug_adapter_protocol(self, dct, fmt=None):
        ...
        from_default_resolver = defaultResolver.get_contents_debug_adapter_protocol(
            dct, fmt
        )
        ...
        ret.append(
            (
                GENERATED_LEN_ATTR_NAME,
                len(dct),
                partial(_apply_evaluate_name, evaluate_name="len(%s)"),
            )
        )
        return ret
...
class SetResolver:
    ...
    def resolve(self, var, attribute):
        ...
        raise UnableToResolveVariableException(
            "Unable to resolve %s in %s" % (attribute, var)
        )
...
class DjangoFormResolver(DefaultResolver):
    def get_dictionary(self, var, names=None):
        ...
        d = defaultResolver.get_dictionary(
            var, names=names, used___dict__=used___dict__
        )
        if has_errors_attr:
            try:

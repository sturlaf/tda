...
class SourceMapping(object):
    def __init__(self, on_source_mapping_changed=NULL):
        self._mappings_to_server = (
            {}
        )  # dict(normalized(file.py) to [SourceMappingEntry])
    ...
    def set_source_mapping(self, absolute_filename, mapping):
        ...
        for map_entry in mapping:
            ...
            if (
                ...
                and existing_source_filename != absolute_filename
            ):
                return (
                    "Cannot apply mapping from %s to %s (it conflicts with mapping: %s to %s)"
                    % (
                        absolute_filename,
                        map_entry.runtime_source,
                        existing_source_filename,
                ...
                for map_entry in mapping:
                    if map_entry.runtime_source == runtime_source_filename:  # <cell1>
                        if map_entry.contains_runtime_line(
                            ...
                            self._cache[key] = (
                                map_entry.source_filename,
                                map_entry.line + (lineno - map_entry.runtime_line),
                                True,
                            )
                            return self._cache[key]

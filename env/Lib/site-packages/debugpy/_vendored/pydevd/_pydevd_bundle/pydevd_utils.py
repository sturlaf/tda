...
if USE_CUSTOM_SYS_CURRENT_FRAMES and IS_PYPY:
    ...
    from _pydevd_bundle.pydevd_constants import (
        _current_frames as _tid_to_frame_for_dump_threads,
    )
...
def dump_threads(stream=None, show_pydevd_threads=True):
    ...
    try:
        for t in threading.enumerate():
            ...
            thread_id_to_name_and_is_pydevd_thread[t.ident] = (
                ...
                % (t.name, t.daemon, is_pydevd_thread),
                is_pydevd_thread,
            )
    except:
        pass
    ...
    for thread_id, frame in _tid_to_frame_for_dump_threads().items():
        ...
        for i, (filename, lineno, name, line) in enumerate(
            ...
            if i == 0 and "self" in frame.f_locals:
                ...
                try:
                    ...
                    stream.write(
                        "Unable to get str of: %s" % (type(frame.f_locals["self"]),)
                    )
                stream.write("\n")
        stream.flush()
...
class ScopeRequest(object):
    ...
    def __eq__(self, o):
        if isinstance(o, ScopeRequest):
            return (
                self.variable_reference == o.variable_reference

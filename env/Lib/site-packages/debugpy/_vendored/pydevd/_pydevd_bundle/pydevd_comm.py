    ...
    def do_it(self, dbg):
        try:
            ...
            var = pydevd_vars.eval_in_context(
                self.name, frame.f_globals, frame.f_locals
            )
    ...
    with ctx:
        if IS_PY2 and isinstance(expression, unicode):
            try:
                ...
                _evaluate_response(
                    py_db, request, "", error_message="Expression is not valid utf-8."
                )
                raise
...
def internal_set_expression_json(py_db, request, thread_id):
    ...
    if IS_PY2 and isinstance(value, unicode):
        try:
            ...
            _evaluate_response(
                py_db, request, "", error_message="Value is not valid utf-8."
            )
            raise
    ...
    if is_error:
        _set_expression_response(
            py_db, request, result, error_message="Error executing: %s" % (exec_code,)
        )
        return
            ...
            try:
                ...
                while frames_list is not None and len(frames_list):
                    ...
                    for (
                        ...
                        frames.append(
                            (filename_in_utf8, lineno, method_name, line_text)
                        )
                    ...
                    frames_list = create_frames_list_from_exception_cause(
                        frames_list.trace_obj,
                        None,
                        frames_list.exc_type,
        ...
        try:
            try:
                ...
                result = pydevconsole.console_exec(
                    self.thread_id, self.frame_id, self.expression, dbg
                )
                xml = "<xml>"
                xml += pydevd_xml.var_to_xml(result, "")
                xml += "</xml>"

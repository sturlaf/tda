...
class RedirectToPyDBIoMessages(object):
    def __init__(self, out_ctx, wrap_stream, wrap_buffer, on_write=None):
        ...
        if wrap_buffer:
            self.buffer = RedirectToPyDBIoMessages(
                out_ctx, wrap_stream, wrap_buffer=False, on_write=on_write
            )
    ...
    with _RedirectionsHolder._lock:
        ...
        for std in config_stds:
            ...
            if keep_original_redirection:
                wrap_buffer = (
                    True if not IS_PY2 and hasattr(redirect_to, "buffer") else False
                )
                new_std_instance = IORedirector(
                    getattr(sys, std), redirect_to, wrap_buffer=wrap_buffer
                )
...
def redirect_stream_to_pydb_io_messages(std):
    ...
    with _RedirectionsHolder._lock:
        ...
        if getattr(_RedirectionsHolder, redirect_to_name) is None:
            ...
            redirect_to = RedirectToPyDBIoMessages(
                1 if std == "stdout" else 2, original, wrap_buffer
            )
            start_redirect(
                keep_original_redirection=True, std=std, redirect_to=redirect_to
            )

...
try:
    ...
    trace._warn = (
        lambda *args: None
    )  # workaround for http://bugs.python.org/issue17143 (PY-8706)
...
def get_type_of_value(
    ...
    if (
        ...
        and tp.__module__
        and tp.__module__ not in ignore_module_name
    ):
...
def create_signature_message(signature):
    ...
    cmdTextList.append(
        ...
        % (
            pydevd_xml.make_valid_xml_value(signature.file),
            pydevd_xml.make_valid_xml_value(signature.name),
        )
    )

    for arg in signature.args:
        cmdTextList.append(
            '<arg name="%s" type="%s"></arg>'
            % (
                pydevd_xml.make_valid_xml_value(arg[0]),
                pydevd_xml.make_valid_xml_value(arg[1]),
            )
        )

    if signature.return_type is not None:
        cmdTextList.append(
            '<return type="%s"></return>'
            % (pydevd_xml.make_valid_xml_value(signature.return_type))
        )
...
def send_signature_return_trace(dbg, frame, filename, return_value):
    if dbg.signature_factory and dbg.in_project_scope(frame):
        signature = dbg.signature_factory.create_signature(
            frame, filename, with_args=False
        )

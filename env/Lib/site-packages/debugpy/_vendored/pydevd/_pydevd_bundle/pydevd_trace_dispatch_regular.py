    ...
    def trace_dispatch_and_unhandled_exceptions(self, frame, event, arg):
        ...
        if event == "exception":
            ...
            try:
                ...
                ):  # Note: only check it here, don't set.
                    if is_unhandled_exception(
                        self, py_db, frame, self._last_raise_line, self._raise_lines
                    ):
                        py_db.stop_on_unhandled_exception(
                            py_db, t, additional_info, self._last_exc_arg
                        )
            finally:
...
class ThreadTracer(object):
    ...
    def __call__(self, frame, event, arg):
        ...
        if additional_info.is_tracing:
            return (
                None if event == "call" else NO_FTRACE
            )  # we don't wan't to trace code invoked from pydevd_frame.trace_dispatch
        ...
        try:
            ...
            if py_db.is_files_filter_enabled:
                if py_db.apply_files_filter(
                    ...
                    if back_frame is not None and pydev_step_cmd in (
                        ...
                        if py_db.apply_files_filter(
                            back_frame, back_frame.f_code.co_filename, False
                        ):
                            back_frame_cache_key = (
                                back_frame.f_code.co_firstlineno,
                                back_frame.f_code.co_name,
            ...
            if py_db.pydb_disposed:
                return (
                    None if event == "call" else NO_FTRACE
                )  # Don't log errors when we're shutting down.
            # Log it
            try:

...
def has_line_breaks(plugin):
    for _canonical_normalized_filename, breakpoints in dict_iter_items(
...
def cmd_step_into(plugin, pydb, frame, event, args, stop_info, stop):
    ...
    if _is_jinja2_suspended(thread):
        stop_info["jinja2_stop"] = event in ("call", "line") and _is_jinja2_render_call(
            frame
        )

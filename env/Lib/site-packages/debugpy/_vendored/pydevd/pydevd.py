...
if sys.version_info[:2] < (2, 6):
    raise RuntimeError(
        "The PyDev.Debugger requires Python 2.6 onwards to be run. If you need to use an older Python version, use an older version of the debugger."
    )
import os
...
from pydevd_concurrency_analyser.pydevd_concurrency_logger import (
    ThreadingLogger,
    AsyncioLogger,
            ...
            if self._suspended_thread_ids:
                if global_suspend_time > self._last_suspend_notification_time:
                    ...
                    self.send_suspend_notification(
                        next(iter(self._suspended_thread_ids)), CMD_THREAD_SUSPEND
                    )
...
class PyDB(object):
    ...
    def __init__(self, set_as_global=True):
        ...
        self.source_mapping = SourceMapping(
            on_source_mapping_changed=self._clear_filters_caches
        )
        ...
        if is_current_thread_main_thread():
            for module in dict_keys(self.mpl_modules_for_patching):
                import_hook_manager.add_module_name(
                    module, self.mpl_modules_for_patching.pop(module)
                )
    ...
    def init_matplotlib_support(self):
        ...
        def return_control():
            ...
            _MatplotlibHelper._return_control_osc = (
                not _MatplotlibHelper._return_control_osc
            )
                ...
                for thread_id in process_thread_ids:
                    ...
                    try:
                        while True:
                            ...
                            if not self.mpl_hooks_in_debug_console and isinstance(
                                int_cmd, InternalConsoleExec
                            ):
                                ...
                                try:
                                    ...
                                    pydev_log.debug(
                                        "Matplotlib support in debug console failed",
                            ...
                            if int_cmd.can_be_executed_by(curr_thread_id):
                                ...
                                pydev_log.verbose(
                                    "NOT processing internal command: %s ", int_cmd
                                )
    ...
    def consolidate_breakpoints(
        self, canonical_normalized_filename, id_to_breakpoint, breakpoints
    ):
        break_dict = {}
    ...
    def do_wait_suspend(
        ...
        with self.suspended_frames_manager.track_frames(self) as frames_tracker:
            ...
            cmd = frames_tracker.create_thread_suspend_command(
                thread_id, stop_reason, message, suspend_type
            )
            self.writer.add_command(cmd)
            ...
            with CustomFramesContainer.custom_frames_lock:  # @UndefinedVariable
                ...
                for frame_custom_thread_id, custom_frame in dict_iter_items(
    ...
    def _do_wait_suspend(
        self, thread, frame, event, arg, suspend_type, from_this_thread, frames_tracker
    ):
        ...
        if info.pydev_step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE):
            ...
            if stop:
                ...
                cmd = self.cmd_factory.make_thread_run_message(
                    get_current_thread_id(thread), info.pydev_step_cmd
                )
                self.writer.add_command(cmd)
            ...
            if wait_for_threads_to_finish_called:
                ...
                try:
                    ...
                    def get_pydb_daemon_threads_to_wait():
                        pydb_daemon_threads = set(
                            dict_keys(self.created_pydb_daemon_threads)
                        )
...
def main():
    ...
    DebugInfoHolder.DEBUG_RECORD_SOCKET_READS = setup.get(
        "DEBUG_RECORD_SOCKET_READS", DebugInfoHolder.DEBUG_RECORD_SOCKET_READS
    )
    DebugInfoHolder.DEBUG_TRACE_BREAKPOINTS = setup.get(
        "DEBUG_TRACE_BREAKPOINTS", DebugInfoHolder.DEBUG_TRACE_BREAKPOINTS
    )
    DebugInfoHolder.DEBUG_TRACE_LEVEL = setup.get(

...
class InterpreterInterface(BaseInterpreterInterface):
    ...
    def notify_about_magic(self):
        if not self.notification_succeeded:
            ...
            if server is not None:
                try:
                    server.NotifyAboutMagic(
                        magic_commands, self.interpreter.is_automagic()
                    )
    ...
    def get_ipython_hidden_vars_dict(self):
        try:
            if hasattr(self.interpreter, "ipython") and hasattr(
                self.interpreter.ipython, "user_ns_hidden"
            ):
                ...
                if isinstance(user_ns_hidden, dict):
                    ...
                    user_hidden_dict = dict(
                        [
                            (key, val)
                            for key, val in dict_iter_items(
                                self.interpreter.ipython.user_ns
                            )

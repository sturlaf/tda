...
class _PyDevFrontEnd:
    ...
    def __init__(self):
        ...
        if (
            hasattr(PyDevTerminalInteractiveShell, "_instance")
            and PyDevTerminalInteractiveShell._instance is not None
        ):
    ...
    def update(self, globals, locals):
        ...
        if hasattr(self.ipython, "history_manager") and hasattr(
            ...
            self.ipython.history_manager.save_thread.pydev_do_not_trace = (
                True  # don't trace ipython history saving thread
            )

    def complete(self, string):
        try:
            if string:
                return self.ipython.complete(
                    None, line=string, cursor_pos=string.__len__()
                )
            else:
...
def get_pydev_frontend(pydev_host, pydev_client_port):
    ...
    if _PyDevFrontEndContainer._last_host_port != (pydev_host, pydev_client_port):
        ...
        _PyDevFrontEndContainer._instance.ipython.hooks["editor"] = create_editor_hook(
            pydev_host, pydev_client_port
        )

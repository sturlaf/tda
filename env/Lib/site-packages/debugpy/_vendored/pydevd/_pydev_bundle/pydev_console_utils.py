...
class BaseInterpreterInterface:
    ...
    def create_std_in(self, debugger=None, original_std_in=None):
        if debugger is None:
            return StdIn(
                self, self.host, self.client_port, original_stdin=original_std_in
            )
        else:
        ...
        try:
            ...
            try:
                ...
                if "pydoc" in sys.modules:
                    pydoc = sys.modules[
                        "pydoc"
                    ]  # Don't import it if it still is not there.
    ...
    def __resolve_reference__(self, text):
        ...
        if "." not in text:
            ...
            try:
                ...
                res = pydevd_vars.eval_in_context(
                    parent_context, self.get_namespace(), self.get_namespace()
                )
    ...
    def getVariable(self, attributes):
        ...
        for k, val in dict_iter_items(val_dict):
            ...
            xml.write(
                pydevd_vars.var_to_xml(val, k, evaluate_full_value=evaluate_full_value)
            )
    ...
    def getArray(self, attr, roffset, coffset, rows, cols, format):
        name = attr.split("\t")[-1]
        array = pydevd_vars.eval_in_context(
            name, self.get_namespace(), self.get_namespace()
        )
        return pydevd_vars.table_like_struct_to_xml(
    ...
    def evaluate(self, expression):
        xml = StringIO.StringIO()
        xml.write("<xml>")
        result = pydevd_vars.eval_in_context(
            expression, self.get_namespace(), self.get_namespace()
        )
    ...
    def connectToDebugger(self, debuggerPort, debugger_options=None):
        ...
        def do_connect_to_debugger():
            ...
            self.debugger.add_fake_frame(
                thread_id=VIRTUAL_CONSOLE_ID, frame_id=VIRTUAL_FRAME_ID, frame=f
            )
            try:

...
def normalize_output(output):
    ...
    if "text/plain" in output.get("data", {}):
        output["data"]["text/plain"] = re.sub(
            addr_pat, "<HEXADDR>", output["data"]["text/plain"]
        )
...
def test_parallel_notebooks(capfd, tmpdir):
    ...
    with modified_env({"NBEXECUTE_TEST_PARALLEL_TMPDIR": str(tmpdir)}):
        threads = [
            threading.Thread(
                target=run_notebook, args=(input_file.format(label=label), opts, res)
            )
            for label in ("A", "B")
        ]
...
def test_many_parallel_notebooks(capfd):
    ...
    with warnings.catch_warnings():
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
            executor.map(
                run_notebook_wrapper, [(input_file, opts, res) for i in range(8)]
            )
...
""",
    ...
    def test_force_raise_errors(self):
        ...
        filename = os.path.join(
            current_dir, "files", "Skip Exceptions with Cell Tags.ipynb"
        )
        res = self.build_resources()
        res["metadata"]["path"] = os.path.dirname(filename)
        with pytest.raises(CellExecutionError) as exc:
    ...
    def test_process_message_wrapper(self):
        ...
        class WrappedPreProc(NotebookClient):
            def process_message(self, msg, cell, cell_index):
                result = super(WrappedPreProc, self).process_message(
                    msg, cell, cell_index
                )
                if result:
...
class TestRunCell(NBClientTestsBase):
    ...
    def test_clear_output_wait_then_message_message(
        self, executor, cell_mock, message_mock
    ):
        executor.execute_cell(cell_mock, 0)
    ...
    def test_clear_output_wait_then_update_display_message(
        self, executor, cell_mock, message_mock
    ):
        executor.execute_cell(cell_mock, 0)
        # An stdout stream, followed by a wait clear, an stderr stream, and then an idle
    ...
    def test_execution_count_message_ignored_on_override(
        self, executor, cell_mock, message_mock
    ):
    ...
    def test_execution_count_with_stream_message(
        self, executor, cell_mock, message_mock
    ):
        executor.execute_cell(cell_mock, 0)
        # An execution count followed by an idle
    ...
    def test_execute_result_with_display_message(
        self, executor, cell_mock, message_mock
    ):
        executor.execute_cell(cell_mock, 0)
        # An execute followed by an idle
    ...
    def test_update_display_data_without_id_message(
        self, executor, cell_mock, message_mock
    ):
        executor.execute_cell(cell_mock, 0)
    ...
    def test_update_display_data_mismatch_id_message(
        self, executor, cell_mock, message_mock
    ):
        executor.execute_cell(cell_mock, 0)
        # An update followed by an idle
    ...
    def test_non_code_cell(self, executor, cell_mock, message_mock):
        cell_mock = NotebookNode(
            source='"foo" = "bar"', metadata={}, cell_type="raw", outputs=[]
        )

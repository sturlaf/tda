...
class TestPercentile:
    ...
    def test_linear(self):
        ...
        assert_equal(np.percentile(range(10), 50, interpolation="linear"), 4.5)

    def test_lower_higher(self):

        # interpolation_method 'lower'/'higher'
        assert_equal(np.percentile(range(10), 50, interpolation="lower"), 4)
        assert_equal(np.percentile(range(10), 50, interpolation="higher"), 5)

    def test_midpoint(self):
        assert_equal(np.percentile(range(10), 51, interpolation="midpoint"), 4.5)
        assert_equal(np.percentile(range(11), 51, interpolation="midpoint"), 5.5)
        assert_equal(np.percentile(range(11), 50, interpolation="midpoint"), 5)

    def test_nearest(self):
        assert_equal(np.percentile(range(10), 51, interpolation="nearest"), 5)
        assert_equal(np.percentile(range(10), 49, interpolation="nearest"), 4)
    ...
    def test_axis(self):
        ...
        assert_equal(np.percentile(x, (25, 50), interpolation="higher").shape, (2,))
        assert_equal(np.percentile(x, (25, 50, 75), interpolation="higher").shape, (3,))
        assert_equal(
            np.percentile(x, (25, 50), axis=0, interpolation="higher").shape,
            (2, 4, 5, 6),
        )
        assert_equal(
            np.percentile(x, (25, 50), axis=1, interpolation="higher").shape,
            (2, 3, 5, 6),
        )
        assert_equal(
            np.percentile(x, (25, 50), axis=2, interpolation="higher").shape,
            (2, 3, 4, 6),
        )
        assert_equal(
            np.percentile(x, (25, 50), axis=3, interpolation="higher").shape,
            (2, 3, 4, 5),
        )
        assert_equal(
            np.percentile(x, (25, 50, 75), axis=1, interpolation="higher").shape,
            (3, 3, 5, 6),
        )
    ...
    def test_scalar_q(self):
        ...
        # test for no empty dimensions for compatibility with old percentile
        x = np.arange(12).reshape(3, 4)
        assert_equal(np.percentile(x, 50, interpolation="lower"), 5.0)
        assert_(np.isscalar(np.percentile(x, 50)))
        r0 = np.array([4.0, 5.0, 6.0, 7.0])
    ...
    def test_exception(self):
        assert_raises(ValueError, np.percentile, [1, 2], 56, interpolation="foobar")
    ...
    def test_percentile_empty_dim(self):
        ...
        assert_array_equal(
            np.percentile(d, 50, axis=2, interpolation="midpoint").shape, (11, 1, 1)
        )
        assert_array_equal(
            np.percentile(d, 50, axis=-2, interpolation="midpoint").shape, (11, 1, 1)
        )
    ...
    def test_no_p_overwrite(self):
        ...
        np.percentile(np.arange(100.0), p, interpolation="midpoint")
    ...
    def test_nan_behavior(self):
        ...
        b[:, 1] = np.nan
        b[:, 2] = np.nan
        assert_equal(np.percentile(a, [0.3, 0.6], (0, 2), interpolation="nearest"), b)
...
class TestQuantile:
    ...
    def test_no_p_overwrite(self):
        ...
        p = p0.copy()
        np.quantile(np.arange(100.0), p, interpolation="midpoint")
    ...
    def test_quantile_monotonic(self):
        ...
        quantile = np.quantile(
            np.array([0, 1, 1, 2, 2, 3, 3, 4, 5, 5, 1, 1, 9, 9, 9, 8, 8, 7]) * 0.1, p0
        )

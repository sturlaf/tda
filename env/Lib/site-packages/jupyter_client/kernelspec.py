...
def _list_kernels_in(dir):
    ...
    for f in os.listdir(dir):
        ...
        if not _is_valid_kernel_name(key):
            warnings.warn(
                "Invalid kernelspec directory name (%s): %s"
                % (_kernel_name_description, path),
                stacklevel=3,
            )
        kernels[key] = path
    return kernels
...
def install_kernel_spec(
    source_dir, kernel_name=None, user=False, replace=False, prefix=None
):
...
install_native_kernel_spec.__doc__ = (
    KernelSpecManager.install_native_kernel_spec.__doc__
)

...
class InstallKernelSpec(JupyterApp):
    ...
    sourcedir = Unicode()
    kernel_name = Unicode(
        "", config=True, help="Install the kernel spec with this name"
    )
    ...
    replace = Bool(
        False, config=True, help="Replace any existing kernel spec with this name."
    )
...
class RemoveKernelSpec(JupyterApp):
    ...
    force = Bool(
        False, config=True, help="""Force removal, don't prompt for confirmation."""
    )
    ...
    subcommands = Dict(
        {
            ...
            "provisioners": (
                ListProvisioners,
                ListProvisioners.description.splitlines()[0],
            ),
        }
    )

...
initial_provisioner_map = {
    ...
    "custom-test-provisioner": (
        "jupyter_client.tests.test_provisioning",
        "CustomTestProvisioner",
    ),
}
...
def mock_get_provisioner(factory, name) -> EntryPoint:
    ...
    if name in initial_provisioner_map:
        return EntryPoint(
            name, initial_provisioner_map[name][0], initial_provisioner_map[name][1]
        )
...
def kpf(monkeypatch):
    ...
    monkeypatch.setattr(
        KernelProvisionerFactory, "_get_provisioner", mock_get_provisioner
    )
...
class TestDiscovery:
    ...
    def test_get_new(self, kpf):
        ...
        assert (
            "new-test-provisioner"
            == kernel.metadata["kernel_provisioner"]["provisioner_name"]
        )
...
class TestRuntime:
    ...
    async def test_default_provisioner_config(self, kpf, all_provisioners):
        ...
        assert (
            async_km.provisioner.config_var_1 == 0
        )  # Not in kernelspec, so default of 0 exists

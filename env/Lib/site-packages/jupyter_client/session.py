...
def msg_header(
    msg_id: str, msg_type: str, username: str, session: "Session"
) -> t.Dict[str, t.Any]:
...
class Session(Configurable):
    ...
    keyfile = Unicode(
        "", config=True, help="""path to file containing execution key."""
    )
    ...
    def _check_packers(self) -> None:
        ...
        try:
            ...
            error_msg = (
                "packer '{packer}' could not serialize a simple message: {e}{jsonmsg}"
            )
            if self.packer == "json":
        ...
        if not isinstance(packed, bytes):
            raise ValueError(
                "message packed to %r, but bytes are required" % type(packed)
            )
        ...
        try:
            ...
            raise ValueError(
                error_msg.format(
                    packer=self.packer, unpacker=self.unpacker, e=e, jsonmsg=jsonmsg
                )
            ) from e

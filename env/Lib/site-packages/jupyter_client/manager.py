...
class KernelManager(ConnectionFileMixin):
    ...
    def add_restart_callback(
        self, callback: t.Callable, event: str = "restart"
    ) -> None:
    ...
    async def _async_restart_kernel(
        self, now: bool = False, newports: bool = False, **kw
    ) -> None:
        ...
        if self._launch_args is None:
            raise RuntimeError(
                "Cannot restart the kernel. " "No previous call to 'start_kernel'."
            )
        else:

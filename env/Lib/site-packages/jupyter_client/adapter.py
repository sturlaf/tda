...
class V5toV4(Adapter):
    ...
    def kernel_info_reply(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            content.get("implementation", "") == "ipython"
            and "implementation_version" in content
        ):
            v4c["ipython_version"] = _version_str_to_list(
        ...
        if "version" in language_info:
            v4c.setdefault(
                "language_version", _version_str_to_list(language_info["version"])
            )
        msg["content"] = v4c
        return msg
...
def adapt(
    msg: Dict[str, Any], to_version: int = protocol_version_info[0]
) -> Dict[str, Any]:

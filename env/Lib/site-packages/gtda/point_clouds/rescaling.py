...
class ConsistentRescaling(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    _hyperparameters = {
        ...
        "neighbor_rank": {"type": int, "in": Interval(1, np.inf, closed="left")},
    }

    def __init__(
        self, metric="euclidean", metric_params=None, neighbor_rank=1, n_jobs=None
    ):
        self.metric = metric
    ...
    def _consistent_rescaling(self, X):
        Xm = pairwise_distances(
            X, metric=self.metric, n_jobs=1, **self.effective_metric_params_
        )

        indices_k_neighbor = np.argsort(Xm)[:, self.neighbor_rank]
        distance_k_neighbor = Xm[np.arange(X.shape[0]), indices_k_neighbor]
        ...
        for i, j in iterator:
            Xc[i, j] = Xm[i, j] / (
                np.sqrt(distance_k_neighbor[i] * distance_k_neighbor[j])
            )
        return Xc + Xc.T
...
class ConsecutiveRescaling(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    def __init__(self, metric="euclidean", metric_params=None, factor=0.0, n_jobs=None):
        self.metric = metric
        self.metric_params = metric_params
        self.factor = factor
        self.n_jobs = n_jobs

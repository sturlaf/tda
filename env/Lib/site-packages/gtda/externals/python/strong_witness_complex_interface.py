from ..modules.gtda_strong_witness_complex import Strong_witness_complex_interface
...
class StrongWitnessComplex:
    ...
    def __init__(self, nearest_landmark_table=None):
        ...
        if nearest_landmark_table is not None:
            self.thisptr = Strong_witness_complex_interface(nearest_landmark_table)
    ...
    def create_simplex_tree(self, max_alpha_square=float("inf"), limit_dimension=-1):
        ...
        if limit_dimension != -1:
            self.thisptr.create_simplex_tree(
                stree_int_ptr, max_alpha_square, limit_dimension
            )
        else:

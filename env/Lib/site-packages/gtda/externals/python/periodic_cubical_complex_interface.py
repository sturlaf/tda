...
class PeriodicCubicalComplex:
    ...
    def __init__(
        ...
        if (
            ...
            and (periodic_dimensions is not None)
            and (perseus_file == "")
        ):
            self.thisptr = Periodic_cubical_complex_base_interface(
                dimensions, top_dimensional_cells, periodic_dimensions
            )
        elif (
            (dimensions is None)
            and (top_dimensional_cells is None)
            and (periodic_dimensions is None)
            and (perseus_file != "")
        ):
    ...
    def persistence(self, homology_coeff_field=11, min_persistence=0):
        ...
        if self.pcohptr is not None:
            del self.pcohptr
        if self.thisptr is not None:
            self.pcohptr = Periodic_cubical_complex_persistence_interface(
                self.thisptr, True
            )
        persistence_result = []
        if self.pcohptr is not None:

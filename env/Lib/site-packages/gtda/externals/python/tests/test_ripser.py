...
def get_dense_distance_matrices(draw):
    ...
    dm = draw(
        arrays(
            dtype=float,
            elements=floats(allow_nan=False, allow_infinity=True, min_value=0),
            shape=(shapes, shapes),
            unique=False,
        )
    )
...
def test_coo_below_diagonal_and_mixed_same_as_above(dm):
    ...
    to_transpose_mask[
        np.random.choice(np.arange(len(_row)), size=len(_row) // 2, replace=False)
    ] = True
...
def test_collapser_with_negative_weights():
    ...
    maxdim = 2
    pd_collapse_dense = ripser(
        dm, metric="precomputed", maxdim=maxdim, collapse_edges=True
    )["dgms"]
    pd_collapse_sparse = ripser(
        dm_sparse, metric="precomputed", maxdim=maxdim, collapse_edges=True
    )["dgms"]
    pd_no_collapse = ripser(
        dm, metric="precomputed", maxdim=maxdim, collapse_edges=False
    )["dgms"]
...
def test_coo_results_independent_of_order():
    ...
    expected = [
        np.array([[0.0, 1.0], [0.0, 1.0], [0.0, 2.0], [0.0, 5.0], [0.0, np.inf]]),

...
def test_simple_csr_example():
    X_ = csr_matrix((tX[2], (tX[0].astype(np.int32), tX[1].astype(np.int32)))).toarray()
...
def test_simple_coo_example():
    coo_ = flag_complex_collapse_edges_coo(tX[0], tX[1], tX[2])
    coo = coo_matrix((coo_[2], (coo_[0], coo_[1])))
    assert check_collapse(coo, [[1, 3, 2]])
...
def test_simple_dense_example():
    data = csr_matrix(
        (tX[2], (tX[0].astype(np.int32), tX[1].astype(np.int32)))
    ).toarray()

...
_AVAILABLE_RIPS_WEIGHTS = {
    "DTM": {
        ...
        "n_neighbors": {"type": Integral, "in": Interval(1, np.inf, closed="left")},
    },
    "general": {
        "p": {"type": Real, "in": [1, 2, np.inf]},
    },
...
class VietorisRipsPersistence(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    **Important note**:
    ...
    def transform(self, X, y=None):
        ...
        X = check_point_clouds(
            X, accept_sparse=True, distance_matrices=self._is_precomputed
        )

        Xt = Parallel(n_jobs=self.n_jobs)(delayed(self._ripser_diagram)(x) for x in X)
    ...
    **Important notes**:
    ...
    def fit(self, X, y=None):
        ...
        if isinstance(self.weights, str) and self.weights != "DTM":
            raise ValueError(
                f"'{self.weights}' passed for `weights` but the "
        ...
        if self.weight_params:
            ...
            validate_params(self.effective_weight_params_, _AVAILABLE_RIPS_WEIGHTS[key])
...
class SparseRipsPersistence(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    **Important note**:
    ...
    def transform(self, X, y=None):
        ...
        Xt = Parallel(n_jobs=self.n_jobs)(delayed(self._gudhi_diagram)(x) for x in X)
...
class EuclideanCechPersistence(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    **Important note**:
    ...
    _hyperparameters = {
        ...
        "coeff": {"type": int, "in": Interval(2, np.inf, closed="left")},
        "max_edge_length": {"type": Real, "in": Interval(0, np.inf, closed="right")},
        "infinity_values": {
            "type": (Real, type(None)),
    ...
    **Important note**:
    ...
    def _flagser_diagram(self, X):
        ...
        Xdgms += flagser_weighted(
            ...
            coeff=self.coeff,
            approximation=self.max_entries,
        )["dgms"]
    ...
    def fit(self, X, y=None):
        ...
        validate_params(
            self.get_params(), self._hyperparameters, exclude=["n_jobs", "filtration"]
        )
    ...
    def transform(self, X, y=None):
        ...
        Xt = Parallel(n_jobs=self.n_jobs)(delayed(self._flagser_diagram)(x) for x in X)

...
def get_preamble_docs(docs):
    re_search = re.search(f"^(.*){inputs_start}", docs, flags=re.DOTALL)
    return re_search.group(1)


def get_inputs_docs(docs):
    re_search = re.search(f"{inputs_start}(.*){inputs_end}", docs, flags=re.DOTALL)
    return re_search.group(1)


def get_outputs_docs(docs):
    re_search = re.search(f"{inputs_end}(.*){outputs_end}", docs, flags=re.DOTALL)
    if re_search is None:
        re_search = re.search(f"{inputs_end}(.*)$", docs, flags=re.DOTALL)
    return re_search.group(1)
...
def make_fit_transform_docs(fit_docs, transform_docs):
    ...
    new_docstring = (
        standard_fit_transform_docs.replace(standard_intro_docs, intro_docs)
...
def adapt_fit_transform_docs(transformermixin_cls):
    ...
    def make_new_fit_transform(original_fit_transform):
        ...
        fit_transform_wrapper.__doc__ = make_fit_transform_docs(
            fit_docs, transform_docs
        )

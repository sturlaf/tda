...
def check_diagrams(X, copy=False):
    ...
    """
    X_array = _check_array_mod(
        X, ensure_2d=False, allow_nd=True, force_all_finite=False, copy=copy
    )
    if X_array.ndim != 3:
...
def _validate_params(parameters, references, rec_name=None):
    ...
    def _validate_params_single(_parameter, _reference, _name):
        ...
        # performed on _parameter directly.
        elif not ((_ref_type in types_tuple) or isinstance(_parameter, types_tuple)):
            ...
            if _parameter is not None:
                if not ((ref_in is None) or _parameter in ref_in):
                    raise ValueError(
                        f"Parameter `{_name}` is {_parameter}, "
    ...
    for name, parameter in parameters.items():
        if name not in references.keys():
            ...
            raise KeyError(
                f"`{name}`{name_extras} is not an available "
        ...
        if ref_type:
            ...
            if ref_of is None:
                ...
                for i, parameter_elem in enumerate(parameter):
                    _validate_params_single(parameter_elem, ref_of, f"{name}[{i}]")
...
def validate_params(parameters, references, exclude=None):
    ...
    parameters_ = {
        key: value for key, value in parameters.items() if key not in exclude_
    }
    ...
    if hasattr(X, "shape") and hasattr(X, "ndim"):
        if X.ndim != 3:
            ...
            raise ValueError(
                ...
                f"sparse matrices. Structure of dimension {X.ndim} passed." + extra_2D
            )
...
def check_collection(X, **kwargs):
    ...
    **kwargs
    ...
    if hasattr(X, "shape") and hasattr(X, "ndim"):
        ...
        for i, x in enumerate(X):
            try:
                xnew = _check_array_mod(x, ensure_2d=False, allow_nd=True, **kwargs_)
                Xnew.append(xnew)
            except ValueError as e:
                has_check_failed = True
                messages.append(f"Entry {i}:\n{e}")

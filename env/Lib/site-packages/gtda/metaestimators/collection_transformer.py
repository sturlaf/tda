...
class CollectionTransformer(BaseEstimator, TransformerMixin):
    ...
    def _validate_transformer(self):
        if not hasattr(self.transformer, "fit_transform"):
            raise TypeError("`transformer` must possess a fit_transform " "method.")
        if not isinstance(self.transformer, BaseEstimator):
            warn(
                "`transformer` is not an instance of "
    ...
    def fit(self, X, y=None):
        ...
        """
        check_collection(
            X, accept_sparse=True, accept_large_sparse=True, force_all_finite=False
        )

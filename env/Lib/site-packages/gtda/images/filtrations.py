...
class HeightFiltration(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    def _calculate_height(self, X):
        ...
        for i in range(len(Xh)):
            Xh[i][np.where(X[i])] = np.dot(
                self.mesh_[np.where(X[i])], self.direction_
            ).reshape((-1,))
    ...
    def fit(self, X, y=None):
        ...
        if self.n_dimensions_ > 3:
            raise ValueError(
                f"Input of `fit` contains arrays of dimension " f"{self.n_dimensions_}."
            )
        validate_params(self.get_params(), self._hyperparameters, exclude=["n_jobs"])

        if self.direction is None:
            self.direction_ = np.ones(
                self.n_dimensions_,
            )
        else:
        ...
        axis_order = [2, 1, 3]
        mesh_range_list = [
            np.arange(X.shape[order])
            if self.direction_[i] >= 0
            else -np.flip(np.arange(X.shape[order]))
            for i, order in enumerate(axis_order[: self.n_dimensions_])
        ]

        self.mesh_ = np.stack(
            np.meshgrid(*mesh_range_list, indexing="xy"), axis=self.n_dimensions_
        )

        self.max_value_ = 0.0
        self.max_value_ = np.max(self._calculate_height(np.ones((1, *X.shape[1:])))) + 1
    ...
    @staticmethod
    def plot(Xt, sample=0, colorscale="greys", origin="upper", plotly_params=None):
...
class RadialFiltration(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    def fit(self, X, y=None):
        ...
        axis_order = [2, 1, 3]
        mesh_range_list = [
            np.arange(0, X.shape[i]) for i in axis_order[: self.n_dimensions_]
        ]

        self.mesh_ = np.stack(
            np.meshgrid(*mesh_range_list), axis=self.n_dimensions_
        ).reshape((-1, self.n_dimensions_))
        self.mesh_ = pairwise_distances(
            self.center_, self.mesh_, metric=self.metric, n_jobs=1, **self.metric_params
        ).reshape(X.shape[1:])
        self.mesh_[self.mesh_ > self.radius] = np.inf

        self.max_value_ = 0.0
        self.max_value_ = np.max(self._calculate_radial(np.ones((1, *X.shape[1:])))) + 1
...
class DensityFiltration(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    def __init__(self, radius=3, metric="euclidean", metric_params={}, n_jobs=None):
        self.radius = radius
        self.metric = metric
        self.metric_params = metric_params
        self.n_jobs = n_jobs
    ...
    def _calculate_density(self, X):
        ...
        for i, j, k in self._iterator:
            Xd += (
                np.roll(np.roll(np.roll(X, k, axis=3), j, axis=2), i, axis=1)
                * self.mask_[self._size + i, self._size + j, self._size + k]
            )
        return Xd
    ...
    def fit(self, X, y=None):
        ...
        self._size = int(
            np.ceil(
                pairwise_distances(
                    [[0]], [[self.radius]], metric=self.metric, **self.metric_params
                )
            )
        )
        # The mask is always 3D but not the iterator.
        self.mask_ = np.ones(tuple(2 * self._size + 1 for _ in range(3)), dtype=bool)

        # Create an iterator for applying the mask to every pixel at once
        iterator_size_list = [
            range(-self._size, self._size + 1)
        ] * self.n_dimensions_ + [[0] for _ in range(3 - self.n_dimensions_)]
        ...
        self.mesh_ = np.stack(np.meshgrid(*mesh_size_list), axis=3).reshape((-1, 3))
        ...
        self.mask_ = pairwise_distances(
            center, self.mesh_, metric=self.metric, n_jobs=1, **self.metric_params
        ).reshape(self.mask_.shape)
        ...
        padding = np.asarray(
            [*[self._size] * self.n_dimensions_, *[0] * (3 - self.n_dimensions_)]
        )

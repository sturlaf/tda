...
class Inverter(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    _hyperparameters = {"max_value": {"type": (bool, Real, type(None))}}
...
class Padder(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    _hyperparameters = {
        "padding": {"type": (np.ndarray, type(None)), "of": {"type": Integral}},
    ...
    def fit(self, X, y=None):
        ...
        validate_params(
            self.get_params(), self._hyperparameters, exclude=["value", "n_jobs"]
        )
    ...
    def transform(self, X, y=None):
        ...
        Xt = Parallel(n_jobs=self.n_jobs)(
            delayed(np.pad)(
                Xt[s], pad_width=self._pad_width, constant_values=self.value
            )
            for s in gen_even_slices(len(Xt), effective_n_jobs(self.n_jobs))
        )
        Xt = np.concatenate(Xt)

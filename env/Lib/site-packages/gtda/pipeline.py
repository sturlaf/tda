...
class Pipeline(pipeline.Pipeline):
    ...
    def _fit(self, X, y=None, **fit_params):
        ...
        fit_transform_resample_one_cached = memory.cache(_fit_transform_resample_one)
        ...
        for step_idx, name, transformer in self._iter(with_final=False):
            ...
            if hasattr(cloned_transformer, "resample") or hasattr(
                cloned_transformer, "fit_transform_resample"
            ):
                if y is None:
                    ...
                    X, y, fitted_transformer = fit_transform_resample_one_cached(
                        cloned_transformer, None, X, y, **fit_params_steps[name]
                    )
            else:
                X, fitted_transformer = fit_transform_one_cached(
...
def _fit_transform_resample_one(transformer_resampler, weight, X, y, **fit_params):
    if hasattr(transformer_resampler, "fit_transform_resample"):
        X_res, y_res = transformer_resampler.fit_transform_resample(X, y, **fit_params)
    else:
        X_res, y_res = transformer_resampler.fit(X, y, **fit_params).transform_resample(
            X, y
        )
    if weight is None:

...
class Labeller(BaseEstimator, TransformerResamplerMixin):
    ...
    _hyperparameters = {
        ...
        "n_steps_future": {"type": int, "in": Interval(1, np.inf, closed="left")},
    }

    def __init__(
        self, size=10, func=np.std, func_params=None, percentiles=None, n_steps_future=1
    ):
        self.size = size
        self.func = func
    ...
    def resample(self, y, X=None):
        ...
        if self.thresholds_ is not None:
            ...
            yr = np.concatenate(
                ...
                + [
                    1 * (yr >= self.thresholds_[i]) * (yr < self.thresholds_[i + 1])
                    for i in range(len(self.thresholds_) - 1)
                ]
                + [1 * (yr >= self.thresholds_[-1])],
                axis=1,
            )

...
def test_labeller_shape():
    size = 4
    labeller = Labeller(
        size=size, func=np.std, func_params={}, percentiles=None, n_steps_future=1
    )
...
def test_labeller_resampled():
    size = 6
    labeller = Labeller(
        size=size, func=np.max, func_params={}, percentiles=None, n_steps_future=1
    )
    x, y = labeller.fit_transform_resample(X, X)
    ...
    x, y = labeller.fit_transform_resample(X, X)
    assert_almost_equal(y, np.array([6, 7, 8, 9, 9, 9, 9, 9, 9, 5, 6, 7, 8, 9]))
    assert len(x) == len(y)
...
def test_labeller_invalid_percentage():
    labeller = Labeller(
        size=6, func=np.max, func_params={}, percentiles=[101], n_steps_future=2
    )
    with pytest.raises(ValueError):

...
signal_embedded_search = np.array(
    [
        [2.0, 2.47942554],
...
signal_embedded_fixed = np.array(
    [
        [2.0, 2.47942554, 2.84147098, 2.99749499, 2.90929743],
...
def test_takens_embedding_optimal_parameters_validate():
    ...
    with pytest.raises(ValueError):
        takens_embedding_optimal_parameters(
            signal, time_delay, dimension, validate=True
        )
...
@pytest.mark.parametrize(
    "parameters_type, expected",
    [("search", signal_embedded_search), ("fixed", signal_embedded_fixed)],
)
...
def test_embedder_resample():
    embedder = SingleTakensEmbedding(
        parameters_type="fixed", time_delay=3, dimension=2, stride=3
    )
    embedder.fit(signal)
...
def test_window_slice_windows():
    ...
    assert_almost_equal(np.stack([X[begin:end] for begin, end in slice_idx]), X_windows)
...
def test_takens_embedding_consistent_with_single_takens_embedding(
    ...
    single_embedder = SingleTakensEmbedding(
...
def test_takens_embedding_plot():
    ...
    TE.fit_transform_plot([np.arange(20)], sample=0, plotly_params=plotly_params)

...
class StandardFeatures(BaseEstimator, TransformerMixin):
    ...
    _hyperparameters = {
        "function": {
            "type": (str, FunctionType, list, tuple),
            "in": tuple(_AVAILABLE_FUNCTIONS.keys()),
            "of": {
                "type": (str, FunctionType, type(None)),
                "in": tuple(_AVAILABLE_FUNCTIONS.keys()),
            },
        },
    ...
    def _validate_params(self):
        ...
        # instances of FunctionType and the "in" key checks fail
            ...
            end_string = f"which is not in " f"{tuple(_AVAILABLE_FUNCTIONS.keys())}."
            function = params["function"]
            if ve.args[0].endswith(end_string) and isinstance(function, (list, tuple)):
                params["function"] = [f for f in function if isinstance(f, str)]
                validate_params(params, _hyperparameters, exclude=["n_jobs"])
            else:
                raise ve

        if isinstance(self.function, (list, tuple)) and isinstance(
            self.function_params, dict
        ):
            raise TypeError(
                "If `function` is a list/tuple then "
                "`function_params` must be a list/tuple of dict, "
                "or None."
            )
        elif isinstance(self.function, (str, FunctionType)) and isinstance(
            self.function_params, (list, tuple)
        ):
            raise TypeError(
                "If `function` is a string or a callable "
    ...
    def fit(self, X, y=None):
        ...
        if isinstance(self.function, str):
            self.effective_function_ = _implemented_function_recipes[self.function]

            if self.function_params is None:
                self.effective_function_params_ = {}
            else:
                validate_params(
                    self.function_params, _AVAILABLE_FUNCTIONS[self.function]
                )
                self.effective_function_params_ = self.function_params.copy()

        elif isinstance(self.function, FunctionType):
            self.effective_function_ = tuple([self.function] * self.n_channels_)

            if self.function_params is None:
                self.effective_function_params_ = tuple([{}] * self.n_channels_)
            else:
                self.effective_function_params_ = tuple(
                    [self.function_params.copy()] * self.n_channels_
                )
        else:
            ...
            for f, p in zip(self.function, self._effective_function_params):
                if isinstance(f, str):
                    ...
                    self.effective_function_.append(_implemented_function_recipes[f])
                else:
                    self.effective_function_.append(f)
                self.effective_function_params_.append({} if p is None else p.copy())
            self.effective_function_ = tuple(self.effective_function_)
            self.effective_function_params_ = tuple(self.effective_function_params_)
    ...
    def transform(self, X, y=None):
        ...
        if Xt.shape[1] != self.n_channels_:
            raise ValueError(
                f"Number of channels must be the same as in "
                f"`fit`. Passed {Xt.shape[1]}, expected "
                f"{self.n_channels_}."
            )

        Xt = _parallel_featurization(
            Xt, self.effective_function_, self.effective_function_params_, self.n_jobs
        )

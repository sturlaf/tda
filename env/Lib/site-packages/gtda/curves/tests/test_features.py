...
def test_standard_transform_channels_different_from_fit_channels():
    ...
    with pytest.raises(
        ValueError, match="Number of channels must be the " "same as in `fit`"
    ):
...
def test_standard_invalid_function_function_params():
    ...
    with pytest.raises(
        TypeError, match="If `function` is a string or a " "callable function"
    ):
        sf.fit(X)
...
@pytest.mark.parametrize(
    "function, function_params, effective_function, " "effective_function_params",
    [
        ("max", None, np.max, {}),
        ("max", {}, np.max, {}),
        (np.max, None, (np.max, np.max), ({}, {})),
        (np.max, {}, (np.max, np.max), ({}, {})),
        ([np.max, np.min], [{}, None], (np.max, np.min), ({}, {})),
        ([np.max, None], [{}, None], (np.max, None), ({}, {})),
    ],
)
def test_standard_fit_attrs(
    function, function_params, effective_function, effective_function_params
):
    ...
    assert (
        sf.effective_function_ == effective_function
...
X_res = {
    ...
    "argmax": np.array([[8, 0], [12, 12], [9, 3]]),
    "argmin": np.array([[16, 14], [3, 15], [19, 8]]),
    "min": np.array(
        [[0.0202184, 0.0187898], [0.06022547, 0.03918779], [0.00469548, 0.22308163]]
    ),
    "max": np.array(
        [[0.96366276, 0.97861834], [0.98837384, 0.97676109], [0.9292962, 0.96218855]]
    ),
    "mean": np.array(
        [[0.58155482, 0.56784552], [0.39983943, 0.40521714], [0.40951229, 0.61738194]]
    ),
    "std": np.array(
        [[0.27591522, 0.26865653], [0.23900448, 0.31701912], [0.27368227, 0.23340901]]
    ),
    "median": np.array(
        [[0.58540397, 0.61451486], [0.36160934, 0.28947358], [0.36641597, 0.63492923]]
    ),
    "average": np.array(
        [[0.58155482, 0.56784552], [0.39983943, 0.40521714], [0.40951229, 0.61738194]]
    ),
...
def test_standard_transform_function_params():
    ...
    weights[0] = 1
    sf = StandardFeatures(function="average", function_params={"weights": weights})
    Xt = sf.fit_transform(X)
    ...
    sf.set_params(
        function=[np.average, np.average],

...
from .utils._cover import _check_has_one_column, _remove_empty_and_duplicate_intervals
...
class OneDimensionalCover(BaseEstimator, TransformerMixin):
    ...
    def _fit_balanced(self, X):
        ...
        right_ranks = np.where(
            right_limits_int == right_limits, right_limits_int, right_limits_int + 1
        )
    ...
    def _fit_transform_balanced(self, X):
        ...
        X_rank = np.broadcast_to(X_rank[:, None], (X.shape[0], self.n_intervals))
    ...
    def _find_interval_limits(self, X, n_intervals, overlap_frac, is_uniform=True):
        if is_uniform:
        ...
        left_limits, right_limits = self._cover_limits(
            min_val, max_val, n_intervals, overlap_frac
        )
        if is_uniform:
    ...
    def _limits_from_ranks(self, X_rank, X, left_ranks, right_ranks):
        ...
        X_rank = np.broadcast_to(X_rank[:, None], (X_rank.shape[0], n_intervals))
        left_mask = X_rank == left_ranks
        right_mask = X_rank == right_ranks
        left_indices = (np.flatnonzero(left_mask[:, i]) for i in range(n_intervals))
        right_indices = (np.flatnonzero(right_mask[:, i]) for i in range(n_intervals))
    ...
    def _cover_limits(min_val, max_val, n_intervals, overlap_frac):
        ...
        interval_len = total_len / (n_intervals - (n_intervals - 1) * overlap_frac)

        last = min_val + (n_intervals - 1) * (1 - overlap_frac) * interval_len
        left_limits = np.linspace(min_val, last, num=n_intervals, endpoint=True)
...
class CubicalCover(BaseEstimator, TransformerMixin):
    ...
    def _clone_and_apply_to_column(self, X, coverer, method_name, i):
        ...
        try:
            ...
            if (
                ve.args[0] == f"Only one unique filter value found, cannot "
    ...
    def _fit(self, X):
        coverer = OneDimensionalCover(
            kind=self.kind, n_intervals=self.n_intervals, overlap_frac=self.overlap_frac
        )
        is_uniform = self.kind == "uniform"
    ...
    def _transform(self, X):
        ...
        covers = [
            coverer._transform(X[:, [i]]) for i, coverer in enumerate(self._coverers)
        ]
    ...
    def fit_transform(self, X, y=None, **fit_params):
        ...
        covers = [
            partial(self._clone_and_apply_to_column, Xt, coverer, fit_transformer)(i)
    ...
    def _combine_one_dim_covers(covers):
        ...
        intervals = ([cover[:, i] for i in range(cover.shape[1])] for cover in covers)

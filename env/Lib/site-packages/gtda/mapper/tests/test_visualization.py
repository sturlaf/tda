...
@pytest.mark.parametrize(
    "color_features",
    [PCA(n_components=2), DummyPCA(n_components=2), DummyPCA(n_components=2).transform],
)
def test_color_features_as_estimator_or_callable(color_features):
    ...
    node_colors_color_features = np.array(
        [np.mean(color_data_transformed[itr, 0]) for itr in node_elements]
    )

    fig = plot_static_mapper_graph(
        pipe, X_arr, color_data=X_arr, color_features=color_features
    )
...
def test_color_features_as_columns_fails_on_series():
    ...
    with pytest.raises(ValueError, match="If `color_data` is a pandas series"):
        plot_static_mapper_graph(pipe, X_df, color_data=X_df["a"], color_features="a")
...
def test_invalid_color_features_types(color_features):
    ...
    with pytest.raises(ValueError):
        plot_static_mapper_graph(pipe, X_arr, color_features=color_features)
...
def test_valid_color_data_transformed(color_data, color_features):
    ...
    pipe = make_mapper_pipeline()
    plot_static_mapper_graph(
        pipe, X_arr, color_data=color_data, color_features=color_features
    )
...
def test_node_color_statistic_as_ndarray(is_2d):
    ...
    if is_2d:
        node_color_statistic = np.vstack(
            [node_color_statistic_col_0, node_color_statistic_col_0]
        ).T
    else:
...
def test_node_color_statistic_as_ndarray_wrong_length():
    ...
    with pytest.raises(ValueError):
        plot_static_mapper_graph(pipe, X_arr, node_color_statistic=node_color_statistic)
...
def test_invalid_color_features_as_array_of_indices():
    ...
    with pytest.raises(ValueError):
        plot_static_mapper_graph(
            pipe, X_arr, color_data=X_arr, color_features=np.arange(X_arr.shape[1])
        )
...
def test_valid_colorscale(X):
    ...
    marker_colorscale_default = fig_default.data[1]["marker"]["colorscale"]
...
def test_colors_same_2d_3d(X, color_data, node_color_statistic):
    ...
    assert np.array_equal(fig_2d.data[1].marker.color, fig_3d.data[1].marker.color)


@pytest.mark.parametrize(
    "X, columns", [(X_arr, range(X_arr.shape[1])), (X_df, X_df.columns)]
)
@pytest.mark.parametrize("layout_dim", [2, 3])
def test_column_dropdown(X, columns, layout_dim):
    pipe = make_mapper_pipeline()
    fig = plot_static_mapper_graph(pipe, X, color_data=X, layout_dim=layout_dim)
    fig_buttons = fig.layout.updatemenus[0].buttons

    assert list(fig.data[1].marker.color) == list(
        fig_buttons[0].args[0]["marker.color"][1]
    )
    ...
    for i, col in enumerate(columns):
        ...
        assert list(fig_col.data[1].marker.color) == list(
            fig_buttons[i].args[0]["marker.color"][1]
        )
...
class TestStaticPlot(TestCaseNoTemplate):
    def test_is_data_present(self):
        ...
        pipe = make_mapper_pipeline()
        fig = plot_static_mapper_graph(
            pipe, X_arr, color_data=colors, clone_pipeline=False
        )
    ...
    def test_cluster_sizes(self):
        ...
        node_sizes_vis = [_get_size_from_hovertext(ht) for ht in node_trace.hovertext]
...
def test_interactive_plotter_attrs(X, color_data, layout_dim):
    ...
    # 5 Test figure_
    static_fig = plot_static_mapper_graph(
        pipe, X, color_data=color_data, layout_dim=layout_dim
    )
    ...
    for attr in edge_trace_attrs:
        assert np.all(
            getattr(interactive_fig.data[0], attr) == getattr(static_fig.data[0], attr)
        )
    ...
    for attr in node_trace_attrs:
        assert np.all(
            getattr(interactive_fig.data[1], attr) == getattr(static_fig.data[1], attr)
        )
    ...
    for attr in marker_attrs:
        assert np.all(
            getattr(interactive_fig.data[1].marker, attr)
...
def test_pipeline_cloned(clone_pipeline):
    ...
    fig = plot_interactive_mapper_graph(pipe, X_arr, clone_pipeline=clone_pipeline)
    ...
    for step, values in params.items():
        if step in ["cover", "clusterer"]:
            for param_name, initial_param_value in values["initial"].items():
                ...
                final_param_value_actual = pipe.get_mapper_params()[
                    f"{step}__{param_name}"
                ]
            ...
            for w in widgets:
                w.set_state({"value": new_param_value})
            final_param_value_actual = pipe.get_mapper_params()[f"{step}"]
            final_param_value_expected = (
                initial_param_value if clone_pipeline else new_param_value
            )
            assert final_param_value_actual == final_param_value_expected
...
def test_user_hoverlabel_bgcolor_interactive_3d():
    ...
    plotter.plot(
        layout_dim=3, plotly_params={"node_trace": {"hoverlabel_bgcolor": "blue"}}
    )

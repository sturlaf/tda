...
def test_equal_interval_length(filter_values, n_intervals, overlap_frac):
    ...
    cover = OneDimensionalCover(
        kind="uniform", n_intervals=n_intervals, overlap_frac=overlap_frac
    )
    ...
    # rounding precision
    decimals = 10
    assert len(set(np.floor((upper_limits - lower_limits) * decimals).tolist())) == 1
...
def get_input_tests_balanced(draw):
    ...
    points = draw(get_filter_values(shape=(nb_in_each_interval * nb_intervals,)))
...
def test_balanced_is_balanced(balanced_cover):
    ...
    cover = OneDimensionalCover(
        kind="balanced", n_intervals=nb_intervals, overlap_frac=0.01
    )
...
def test_filter_values_covered_by_interval_union(filter_values, n_intervals):
    ...
    interval_masks = cover.fit_transform(filter_values)
    intervals = [
        filter_values[interval_masks[:, i]] for i in range(interval_masks.shape[1])
    ]
    intervals_union = reduce(np.union1d, intervals)
    filter_values_union = filter_values[np.in1d(filter_values, intervals_union)]
    assert_almost_equal(filter_values_union, filter_values)


@given(
    pts=get_filter_values(),
    n_intervals=get_nb_intervals(),
    overlap_frac=get_overlap_fraction(),
    kind=get_kind(),
)
def test_fit_transform_against_fit_and_transform(pts, n_intervals, kind, overlap_frac):
    """Fitting and transforming should give the same result as fit_transform"""
    cover = OneDimensionalCover(
        n_intervals=n_intervals, kind=kind, overlap_frac=overlap_frac
    )
    x_fit_transf = cover.fit_transform(pts)

    cover2 = OneDimensionalCover(
        n_intervals=n_intervals, kind=kind, overlap_frac=overlap_frac
    )
...
def test_fit_transform_limits_not_computed():
    ...
    cover = OneDimensionalCover(n_intervals=10, kind="balanced", overlap_frac=0.3)
    x = np.arange(0, 30)
...
@given(pts=get_filter_values(shape=array_shapes(min_dims=2, max_dims=2, min_side=2)))

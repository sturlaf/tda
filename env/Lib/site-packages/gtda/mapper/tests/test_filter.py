...
def test_entropy_values_for_negative_inputs(X):
    ...
    with warnings.catch_warnings():
        ...
        entropies = -np.einsum(
            "ij,ij->i", probs, np.where(probs != 0, np.log2(probs), 0)
        )
...
@given(
    X=arrays(
        dtype=float,
        elements=floats(
            allow_nan=False, allow_infinity=False, min_value=1, max_value=1e3
        ),
        shape=array_shapes(min_dims=2, max_dims=2, min_side=2),
    )
)
def test_entropy_values_for_positive_inputs(X):
    ...
    Xt = entropy.fit_transform(X)
    probs = X / X.sum(axis=1, keepdims=True)
    entropies = -np.einsum("ij,ij->i", probs, np.where(probs != 0, np.log2(probs), 0))
    assert_almost_equal(Xt, entropies[:, None])
...
def test_projection_values_equal_slice(X):
    ...
    columns = np.random.choice(X.shape[1], 1 + np.random.randint(X.shape[1] - 1))
...
def test_gaussian_density_values(X):
    ...
    kde_actual = method_to_transform(KernelDensity, "score_samples")(
        bandwidth=np.std(X)
    )
...
def test_list_feature_union_transform(X):
    ...
    list_dim = [0, 1]
    p_1_2 = ListFeatureUnion(
        [("proj" + str(k), Projection(columns=k)) for k in list_dim]
    )

...
def test_parallel_clustering_bad_input():
    ...
    with pytest.raises(
        ValueError, match="`X_tot` and `masks` must have the same number"
    ):
        pc.fit(X)
...
def test_parallel_clustering_metric_affinity_precomputed_not_implemented():
    ...
    with pytest.raises(NotImplementedError, match="Behaviour when metric and affinity"):
        pc.fit(X)
...
def get_clusters(draw, n_clusters, n_points_per_cluster, dim, std=1):
    ...
    positions += np.repeat(np.arange(0, n_clusters).reshape(-1, 1), repeats=dim, axis=1)
    positions = np.repeat(positions, repeats=n_points_per_cluster, axis=0)
    positions += std * draw(get_one_cluster(n_clusters * n_points_per_cluster, dim))
    return positions


@composite
def get_input(draw, n_clusters=None, n_points_per_cluster=None, dim=None, std=0.02):
    ...
    return (
        ...
        n_clusters,
        dim,
        draw(get_clusters(n_clusters, n_points_per_cluster, dim, std=std)),
    )
...
@given(
    inp=get_input(),
    max_frac=floats(min_value=0.0, exclude_min=True, max_value=1.0, exclude_max=False),
)
...
def test_precomputed_distances(inp):
    ...
    assert get_partition_from_preds(preds) == get_partition_from_preds(preds_mat)

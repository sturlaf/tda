...
def test_contract_nodes():
    ...
    for i in range(m.shape[1] - 1):
        ...
        idx_to_remove += list(np.flatnonzero((min_p <= p) & (p <= min_p + gap)))
        idx_to_remove += list(np.flatnonzero((max_p - gap <= p) & (p <= max_p)))

    X_f = X[[x for x in range(len(X)) if x not in idx_to_remove]]

    clusterer = DBSCAN(eps=0.05)
    pipe = make_mapper_pipeline(
        filter_func=filter_func, cover=cover, clusterer=clusterer, contract_nodes=True
    )

...
clust_params = ("clusterer", "n_jobs", "parallel_backend_prefer")
...
class MapperPipeline(Pipeline):
    ...
    def get_mapper_params(self, deep=True):
        ...
        return {
            **{param: pipeline_params[param] for param in global_pipeline_params},
            **self._clean_dict_keys(pipeline_params, nodes_params_prefix),
            **self._clean_dict_keys(pipeline_params, clust_prepr_params_prefix),
    ...
    def set_params(self, **kwargs):
        ...
        mapper_clust_prepr_kwargs = self._subset_kwargs(kwargs, clust_prepr_params)
        ...
        if mapper_clust_prepr_kwargs:
            super().set_params(
                **{
                    clust_prepr_params_prefix + key: mapper_clust_prepr_kwargs[key]
                    for key in mapper_clust_prepr_kwargs
                }
            )
    ...
    def _subset_kwargs(kwargs, param_strings):
        return {
            key: value for key, value in kwargs.items() if key.startswith(param_strings)
        }
...
def make_mapper_pipeline(
    ...
    map_and_cover = Pipeline(
        steps=[("scaler", _scaler), ("filter_func", _filter_func), ("cover", _cover)],
        verbose=verbose,
    )
    ...
    mapper_pipeline = MapperPipeline(steps=all_steps, memory=memory, verbose=verbose)

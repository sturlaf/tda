...
class ParallelClustering(BaseEstimator):
    ...
    def _validate_clusterer(self):
        ...
        if not isinstance(self.clusterer, ClusterMixin):
            raise TypeError(
                "`clusterer` must be an instance of " "sklearn.base.ClusterMixin."
            )
        params = [
            param
            for param in ["metric", "affinity"]
            if param in signature(self.clusterer.__init__).parameters
        ]
        precomputed = [
            (getattr(self.clusterer, param) == "precomputed") for param in params
        ]
        if not precomputed:
            ...
            raise NotImplementedError(
                "Behaviour when metric and affinity "
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if len(X_tot) != len(masks):
            raise ValueError(
                "`X_tot` and `masks` must have the same number " "of rows."
            )
        ...
        labels_single = Parallel(
            n_jobs=self.n_jobs, prefer=self.parallel_backend_prefer
        )(
            delayed(self._labels_single)(
                X_tot[self._indices_computer(rel_indices)], rel_indices, sample_weight
            )
        ...
        for i, (rel_indices, partial_labels) in enumerate(labels_single):
            ...
            labels_i[:] = [((i, partial_label),) for partial_label in partial_labels]
...
class Agglomerative:
    ...
    def _build_tree(self, X):
        ...
        if self.linkage == "ward" and self.affinity != "euclidean":
            raise ValueError(
                f"{self.affinity} was provided as affinity. "
                f"Ward can only work with Euclidean distances."
            )
        if self.linkage not in _TREE_BUILDERS:
            raise ValueError(
                f"Unknown linkage type {self.linkage}. Valid "
...
class FirstSimpleGap(ClusterMixin, BaseEstimator, Agglomerative):
    ...
    _hyperparameters = {
        "linkage": {"type": str},
        "affinity": {"type": str},
        "relative_gap_size": {"type": Real, "in": Interval(0, 1, closed="right")},
    ...
    def fit(self, X, y=None):
        ...
        """
        X = check_array(X)
        validate_params(self.get_params(), self._hyperparameters, exclude=["memory"])
...
class FirstHistogramGap(ClusterMixin, BaseEstimator, Agglomerative):
    ...
    _hyperparameters = {
        "linkage": {"type": str},
        "affinity": {"type": str},
        "freq_threshold": {"type": int, "in": Interval(0, np.inf, closed="left")},
        "max_fraction": {"type": Real, "in": Interval(0, 1, closed="right")},
    ...
    def fit(self, X, y=None):
        ...
        self.n_clusters_ = _num_clusters_histogram(
            self.distances_, self.freq_threshold, self.n_bins_start, self.max_fraction
        )

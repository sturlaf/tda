...
class Nerve(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, min_intersection=1, store_edge_elements=False, contract_nodes=False
    ):
    ...
    def fit_transform(self, X, y=None):
        ...
        labels_to_indices = {
            key: np.array(value) for key, value in labels_to_indices.items()
        }
        ...
        node_index_pairs, weights, intersections, mapping = self._generate_edge_data(
            nodes, n_nodes
        )
        graph.es["weight"] = 1
        ...
        if self.contract_nodes:
            ...
            graph.contract_vertices(limit_mapping, combine_attrs="first")
            graph.delete_vertices(
                [i for i in graph.vs.indices if i != limit_mapping[i]]
            )
    ...
    def _generate_edge_data(self, nodes, n_nodes):
        ...
        def _metadata_append(
            _node_1_idx, _node_2_idx, _intersection_size, _intersection, *args
        ):
        ...
        for node_tuple in filterfalse(_in_same_pullback_set, node_tuples):
            (
                ...
                (node_2_idx, (_, _, node_2_elements)),
            ) = node_tuple

...
def _set_node_sizeref(node_sizes, node_scale=12):
    ...
    return 2.0 * max(node_sizes) / (node_scale ** 2)
...
def _get_node_statistics(color_data_transformed, node_elements, node_color_statistic):
    n_columns = color_data_transformed.shape[1]

    return np.array(
        [
            [
                node_color_statistic(color_data_transformed[itr, i])
...
def _get_column_color_buttons(
    ...
    for column in range(1, len(column_names_dropdown)):
        ...
        hovertext = list(
            map(replace_summary_statistic, hovertext_color_features, node_colors)
        )

        new_button = {
            "args": [
                {"marker.color": [None, node_colors], "hovertext": [None, hovertext]}
            ],
        ...
        if colorscale_for_hoverlabel is not None:
            ...
            new_button["args"][0]["hoverlabel.bgcolor"] = [
                ...
                _get_colors_for_vals(
                    node_colors, min_col, max_col, colorscale_for_hoverlabel
                ),
            ]
...
def _infer_color_features_kind(color_features):
    ...
    if hasattr(color_features, "dtype") or hasattr(color_features, "dtypes"):
        raise ValueError(
            "`color_features` should not be a numpy array or " "pandas dataframe."
        )
...
def _validate_color_kwargs(
    graph, data, color_data, color_features, node_color_statistic, interactive=False
):
    ...
    if interactive:
        if not is_node_color_statistic_callable:
            raise ValueError(
                "`node_color_statistic` must be a callable for " "interactive plots."
            )
    else:
        node_elements = graph.vs["node_elements"]
        if is_node_color_statistic_ndarray:
            node_color_statistic = check_array(node_color_statistic, ensure_2d=False)
            len_colors = len(node_color_statistic)
            n_nodes = len(node_elements)
            if len_colors != n_nodes:
                raise ValueError(
                    f"`node_color_statistic` must have as many "
            ...
            if len(node_color_statistic.shape) > 1:
                ...
                node_color_statistic = node_color_statistic.reshape((len_colors, -1))
        elif not is_node_color_statistic_callable:
            raise ValueError(
                "`node_color_statistic` must be a callable or "
    ...
    if color_features_kind == "fit_transformer":
        ...
        if is_color_data_dataframe:
            ...
            column_names_dropdown = getattr(color_data_transformed, "columns", None)
            color_data_transformed = color_data_transformed.to_numpy()
    ...
    color_data_transformed = color_data_transformed.reshape(
        (len(color_data_transformed), -1)
    )

    return (color_data_transformed, column_names_dropdown, node_color_statistic)
...
def _calculate_graph_data(
    ...
    if not hasattr(node_color_statistic, "dtype"):
        node_colors_color_features = _get_node_statistics(
            color_data_transformed, node_elements, node_color_statistic
        )
    else:
    ...
    node_colors_round = map(
        partial(_round_to_n_sig_figs, n=n_sig_figs), node_colors_color_features[:, 0]
    )
    ...
    if layout_dim not in [2, 3]:
        raise ValueError(f"`layout_dim` must be either 2 or 3. {layout_dim} entered.")
    ...
    edge_x = list(
        reduce(
            iconcat,
            map(
                lambda e: [node_pos[e.source, 0], node_pos[e.target, 0], None], graph.es
            ),
            [],
        )
    )
    edge_y = list(
        reduce(
            iconcat,
            map(
                lambda e: [node_pos[e.source, 1], node_pos[e.target, 1], None], graph.es
            ),
            [],
        )
    )
    ...
    if layout_dim == 2:
        ...
        edge_trace = go.Scatter(x=edge_x, y=edge_y, **plot_options["edge_trace"])
        ...
        edge_z = list(
            reduce(
                iconcat,
                map(
                    lambda e: [node_pos[e.source][2], node_pos[e.target][2], None],
                    graph.es,
                ),
                [],
            )
        ...
        if compute_hoverlabel_bgcolor:
            ...
            try:
                ...
                if e.args[0] == "This colorscale is not supported.":
                    warn(
                        "Data-dependent background hoverlabel colors cannot "
                    ...
                    warn(
                        "Something went wrong in generating data-dependent "
                ...
                colorscale_for_hoverlabel = None
            fig.update_traces(
                hoverlabel_bgcolor=hoverlabel_bgcolor, selector={"name": "node_trace"}
            )
...
def _get_colors_for_vals(vals, vmin, vmax, colorscale, return_hex=True):
    ...
    if colors[0][:3] == "rgb":
        colors = np.asarray([literal_eval(color[3:]) for color in colors], dtype=float)
    elif colors[0][0] == "#":

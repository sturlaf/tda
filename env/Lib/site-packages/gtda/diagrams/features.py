...
class PersistenceEntropy(BaseEstimator, TransformerMixin):
    ...
    **Important notes**:
    ...
    def __init__(self, normalize=False, nan_fill_value=-1.0, n_jobs=None):
...
class ComplexPolynomial(BaseEstimator, TransformerMixin):
    ...
    **Important note**:
    ...
    _hyperparameters = {
        "n_coefficients": {
            "type": (int, type(None), list),
            "in": Interval(1, np.inf, closed="left"),
            "of": {"type": int, "in": Interval(1, np.inf, closed="left")},
        },
        "polynomial_type": {"type": str, "in": _AVAILABLE_POLYNOMIALS.keys()},
    }
    ...
    def fit(self, X, y=None):
        ...
        # Find the unique homology dimensions in the 3D array X passed to `fit`
        # assuming that they can all be found in its zero-th entry
        homology_dimensions_fit, counts = np.unique(X[0, :, 2], return_counts=True)
        self.homology_dimensions_ = _homology_dimensions_to_sorted_ints(
            homology_dimensions_fit
        )

        _n_homology_dimensions = len(self.homology_dimensions_)
        _homology_dimensions_counts = dict(zip(homology_dimensions_fit, counts))
        if self.n_coefficients is None:
            ...
            self.n_coefficients_ = [self.n_coefficients] * _n_homology_dimensions

        self._polynomial_function = _implemented_polynomial_recipes[
    ...
    def _complex_polynomial(self, X, n_coefficients):
        ...
        Xt[n_coefficients : n_coefficients + dimension] = coefficients[:dimension].imag

...
def test_fit_transform_plot_one_hom_dim(transformer, hom_dim_idx):
    plotly_params = {"trace": heatmap_trace_params, "layout": layout_params}
    transformer.fit_transform_plot(
        X, sample=0, homology_dimension_idx=hom_dim_idx, plotly_params=plotly_params
    )


@pytest.mark.parametrize(
    "transformer", [BettiCurve(), PersistenceLandscape(), Silhouette()]
)
@pytest.mark.parametrize("hom_dims", [None, (0,), (1,), (0, 1)])
def test_fit_transform_plot_many_hom_dims(transformer, hom_dims):
    plotly_params = {"traces": line_plots_traces_params, "layout": layout_params}
    transformer.fit_transform_plot(
...
def test_cp_transform(n_coefficients):
    ...
    diagram_res = np.array([[-2.0, -3.0, -4.0, 2.0, -6.0, -28.0, -12.0, 36.0]])
    ...
    diagram_res = np.array(
        [
            [
                -np.sqrt(2 / 13),
                -3 / (2 * np.sqrt(13)),
                (-3 - np.sqrt(13)) / np.sqrt(26),
                1 / np.sqrt(13),
                -2 * (np.sqrt(2 / 13) + 1 / np.sqrt(5)),
                -np.sqrt(8 / (13 * 5)) * 7,
                -3 * (np.sqrt(2 / 13) + 2 / np.sqrt(5)),
                np.sqrt(8 / (13 * 5)) * 9,
            ]
        ]
    )
    ...
    u_01, v_01 = (np.cos(1) - np.sin(1), np.cos(1) + np.sin(1))
    u_02, v_02 = (
    ...
    diagram_res = np.array(
        [
            [
                -1 / 2 * (u_01 + u_02),
                1 / 4 * (u_01 * u_02 - v_01 * v_02),
                -1 / 2 * (v_01 + v_02),
                1 / 4 * (u_01 * v_02 + u_02 * v_01),
                -(u_11 + 2 * u_12),
                2 * (u_11 * u_12 - v_11 * v_12),
                -(v_11 + 2 * v_12),
                2 * (u_11 * v_12 + u_12 * v_11),
            ]
        ]
    )
    assert_almost_equal(cp.fit_transform(X), diagram_res)
...
def test_pi_zero_weight_function(n_jobs):
    pi = PersistenceImage(weight_function=lambda x: x * 0.0, n_jobs=n_jobs)
...
def test_pi_null(X):
    ...
    n_bins = 10
    X = np.append(X, 1 + X[-1])
    diagrams = np.expand_dims(np.stack([X, X, np.zeros(len(X))]).transpose(), axis=0)
...
def test_pi_positive(pts):
    diagrams = np.expand_dims(
        np.concatenate([np.sort(pts, axis=1), np.zeros((pts.shape[0], 1))], axis=1),
        axis=0,
    )
    sigma = (np.max(diagrams[:, :, 1] - np.min(diagrams[:, :, 0]))) / 2
    pi = PersistenceImage(sigma=sigma)
    assert np.all(pi.fit_transform(diagrams) >= 0.0)
...
def test_silhouette_transform(n_jobs):
    sht = Silhouette(n_bins=31, power=1.0, n_jobs=n_jobs)
    X_sht_res = np.array(
        [
            0.0,
            0.05,
...
def test_silhouette_big_order(n_jobs):
    ...
    sht_10 = Silhouette(n_bins=41, power=10.0, n_jobs=n_jobs)
    X_sht_res = np.array(
        [
            0.0,
            0.00170459,
...
pts_gen = arrays(
    dtype=float,
    elements=floats(
        allow_nan=False, allow_infinity=False, min_value=1.0, max_value=10.0
    ),
    shape=(1, 20, 2),
    unique=True,
)
dims_gen = arrays(
    dtype=int, elements=integers(min_value=0, max_value=3), shape=(1, 20, 1)
)
...
def test_hk_positive(pts, dims):
    ...
    assert np.all((np.tril(X_t[:, :, ::-1, :]) + 1e-13) >= 0.0)

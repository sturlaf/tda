...
_AVAILABLE_METRICS = {
    "bottleneck": {"delta": {"type": Real, "in": Interval(0, 1, closed="both")}},
    "wasserstein": {
...
for _metric, _metric_params in _AVAILABLE_METRICS.items():
    if _metric not in ["bottleneck", "wasserstein"]:
        ...
        _AVAILABLE_AMPLITUDE_METRICS[_metric] = {
            name: descr for name, descr in _metric_params.items() if name != "delta"
        }
    ...
    return np.array(
        [
            [
                ...
                for diagram_2 in diagrams_2
            ]
            for diagram_1 in diagrams_1
        ]
    )
...
def bottleneck_amplitudes(diagrams, **kwargs):
    half_lifetimes = (diagrams[:, :, 1] - diagrams[:, :, 0]) / 2.0

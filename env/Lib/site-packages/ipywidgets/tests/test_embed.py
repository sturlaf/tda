...
class CaseWidget(Widget):
    ...
    a = traitlets.Instance(Widget, allow_none=True).tag(
        sync=True, **widget_serialization
    )
    b = traitlets.Instance(Widget, allow_none=True).tag(
        sync=True, **widget_serialization
    )
...
class TestEmbed:
    ...
    def test_cors(self):
        ...
        assert (
            len(re.findall(" crossorigin", code)) == 1
        )  # 1 is from the require, which is ok
        f = StringIO()
        embed_minimal_html(f, w, cors=False, requirejs=True)
        assert (
            len(re.findall(" crossorigin", f.getvalue())) == 1
        )  # 1 is from the require, which is ok
    ...
    def test_escape(self):
        ...
        assert (
            code.find(
                r"\u003cscript A> \u003cScRipt> \u003c/Script> \u003c!-- --> <b>hi</b>"
            )
            >= 0
        )

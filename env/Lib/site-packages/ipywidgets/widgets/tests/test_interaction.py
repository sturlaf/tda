...
from ipywidgets import interact, interact_manual, interactive, interaction, Output
...
def test_default_values():
    @annotate(
        n=10, f=(0, 10.0), g=5, h=OrderedDict([("a", 1), ("b", 2)]), j=["hi", "there"]
    )
    ...
    check_widgets(
        ...
        h=dict(
            cls=widgets.Dropdown, options=OrderedDict([("a", 1), ("b", 2)]), value=2
        ),
        j=dict(cls=widgets.Dropdown, options=("hi", "there"), value="there"),
    )


def test_default_out_of_bounds():
    @annotate(f=(0, 10.0), h={"a": 1}, j=["hi", "there"])
...
def test_float_range_logic():
    ...
    w.upper = 0.4
    check_widget(w, cls=frsw, value=(0.2, 0.4), min=0.0, max=0.6)
    w.value = (0.0, 0.1)  # lower non-overlapping range
    check_widget(w, cls=frsw, value=(0.0, 0.1), min=0.0, max=0.6)
    w.value = (0.5, 0.6)  # upper non-overlapping range
...
def test_state_schema():
    ...
    with open(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../../", "state.schema.json"
        )
    ) as f:
        schema = json.load(f)

...
def bytes_serializer(instance, widget):
    return {
        "data": bytearray(memoryview(instance.data).tobytes())
        if instance.data
        else None
    }
...
class DataWidget(SimpleWidget):
    d = Instance(DataInstance).tag(
        sync=True, to_json=mview_serializer, from_json=deserializer
    )
...
class TruncateDataWidget(SimpleWidget):
    d = Instance(DataInstance).tag(
        sync=True, to_json=bytes_serializer, from_json=truncate_deserializer
    )
    ...
    assert w.comm.messages == [
        (
            ...
            dict(
                buffers=[],
                data=dict(
                    buffer_paths=[], method="update", state=dict(d=[False, True, False])
                ),
            ),
        )
    ]
...
def test_set_state_data_truncate():
    ...
    assert msg == (
        (),
        dict(
            data=dict(buffer_paths=[["d", "data"]], method="update", state=dict(d={}))
        ),
    )

...
class LayoutProperties(HasTraits):
    ...
    def __init__(self, **kwargs):
        ...
        self._property_rewrite["align_items"] = {
            "top": "flex-start",
            "bottom": "flex-end",
        }
...
class AppLayout(GridBox, LayoutProperties):
    ...
    # extra args
    pane_widths = Tuple(
        CUnicode(), CUnicode(), CUnicode(), default_value=["1fr", "2fr", "1fr"]
    )
    pane_heights = Tuple(
        CUnicode(), CUnicode(), CUnicode(), default_value=["1fr", "3fr", "1fr"]
    )
    ...
    def _update_layout(self):
        ...
        if self.merge:
            ...
            if len(children) == 1:
                ...
                if (
                    self.left_sidebar is None
                    and self.right_sidebar is None
        ...
        if self.merge:
            ...
            if len(children) == 1:
                ...
                for i, column in enumerate(columns):
                    top, bottom = children.get("top-" + column), children.get(
                        "bottom-" + column
                    )
                    i_neighbour = (i + 1) % 2

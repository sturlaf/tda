...
widget_serialization = {"from_json": _json_to_widget, "to_json": _widget_to_json}
...
class WidgetRegistry(object):
    ...
    def register(
        self,
        model_module,
        model_module_version_range,
        model_name,
        view_module,
        view_module_version_range,
        view_name,
        klass,
    ):
        """Register a value"""
...
def register(name=""):
    ...
    def reg(widget):
        ...
        Widget.widget_types.register(
            w["_model_module"].default_value,
...
class Widget(LoggingHasTraits):
    ...
    def handle_comm_opened(comm, msg):
        ...
        widget_class = Widget.widget_types.get(
            state["_model_module"],
    ...
    # Traits
    # -------------------------------------------------------------------------
    _model_name = Unicode("WidgetModel", help="Name of the model.", read_only=True).tag(
        sync=True
    )
    _model_module = Unicode(
        "@jupyter-widgets/base", help="The namespace for the model.", read_only=True
    ).tag(sync=True)
    ...
    _view_name = Unicode(None, allow_none=True, help="Name of the view.").tag(sync=True)
    _view_module = Unicode(
        None, allow_none=True, help="The namespace for the view."
    ).tag(sync=True)
    _view_module_version = Unicode(
        "", help="A semver requirement for the namespace version containing the view."
    ).tag(sync=True)
    ...
    def send_state(self, key=None):
        ...
        if len(state) > 0:
            if (
                self._property_lock
            ):  # we need to keep this dict up to date with the front-end values
    ...
    def _should_send_property(self, key, value):
        ...
        if key in self._property_lock:
            ...
            if (
                jsonloads(jsondumps(split_value[0])) == split_lock[0]
    ...
    def _repr_keys(self):
        ...
        for key in sorted(self.keys):
            ...
            if self._compare(value, trait.default_value):
                ...
                isinstance(trait, (Container, Dict))
    ...
    def _gen_repr_from_keys(self, keys):
        ...
        signature = ", ".join("%s=%r" % (key, getattr(self, key)) for key in keys)

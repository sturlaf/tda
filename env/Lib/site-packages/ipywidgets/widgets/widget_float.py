...
class FloatText(_Float):
    ...
    step = CFloat(
        None, allow_none=True, help="Minimum step to increment the value"
    ).tag(sync=True)
...
class FloatSlider(_BoundedFloat):
    ...
    readout_format = NumberFormat(".2f", help="Format for the readout").tag(sync=True)
    continuous_update = Bool(
...
class FloatLogSlider(_BoundedLogFloat):
    ...
    step = CFloat(0.1, help="Minimum step in the exponent to increment the value").tag(
        sync=True
    )
    orientation = CaselessStrEnum(
        values=["horizontal", "vertical"],
    ...
    readout_format = NumberFormat(".3g", help="Format for the readout").tag(sync=True)
    continuous_update = Bool(
        True, help="Update the value of the widget as the user is holding the slider."
    ).tag(sync=True)
    disabled = Bool(False, help="Enable or disable user changes").tag(sync=True)
    base = CFloat(10.0, help="Base for the logarithm").tag(sync=True)
...
class _BoundedFloatRange(_FloatRange):
    step = CFloat(
        1.0, help="Minimum step that the value can take (ignored by some views)"
    ).tag(sync=True)
    max = CFloat(100.0, help="Max value").tag(sync=True)
    min = CFloat(0.0, help="Min value").tag(sync=True)

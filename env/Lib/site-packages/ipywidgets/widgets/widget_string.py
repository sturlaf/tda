...
class _String(DescriptionWidget, ValueWidget, CoreWidget):
    ...
    placeholder = Unicode(
        u"\u200b", help="Placeholder text to display when nothing has been typed"
    ).tag(sync=True)
...
class Textarea(_String):
    ...
    rows = Int(None, allow_none=True, help="The number of rows to display.").tag(
        sync=True
    )
...
class Combobox(Text):
    ...
    options = TypedTuple(trait=Unicode(), help="Dropdown options for the combobox").tag(
        sync=True
    )

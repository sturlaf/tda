...
class SliderStyle(DescriptionStyle, CoreWidget):
    ...
    handle_color = Color(None, allow_none=True, help="Color of the slider handle.").tag(
        sync=True
    )
...
class IntSlider(_BoundedInt):
    ...
    readout = Bool(
        True, help="Display the current value of the slider next to it."
    ).tag(sync=True)
    readout_format = NumberFormat("d", help="Format for the readout").tag(sync=True)
    continuous_update = Bool(
...
class ProgressStyle(DescriptionStyle, CoreWidget):
    ...
    bar_color = Color(None, allow_none=True, help="Color of the progress bar.").tag(
        sync=True
    )
...
class _IntRange(_Int):
    value = Tuple(
        CInt(), CInt(), default_value=(0, 1), help="Tuple of (lower, upper) bounds"
    ).tag(sync=True)
...
class Play(_BoundedInt):
    ...
    _repeat = Bool(help="Whether the control will repeat in a continous loop.").tag(
        sync=True
    )
    show_repeat = Bool(True, help="Show the repeat toggle button in the widget.").tag(
        sync=True
    )
